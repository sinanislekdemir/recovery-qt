...
int main(int argc, char *argv[]) {
...

  {
    ...
    if (!ok) {
      QString msg = QStringLiteral("Cannot create %1 (errno=%2)").arg(LOGFILE).arg(log_errno);
      qDebug() << msg;
...

  {
    ...
    if (!writeLock()) {
      QMessageBox::critical(nullptr, "recovery-qt", "Failed to create lock file " LOCKFILE);
      return 1;
    }
  }
