...
static int align_pread(int (*fnct_pread)(const disk_t *disk_car, void *buf, const unsigned int count,
  ...
  if (count != count_new ||
    ...
    res =
        fnct_pread(disk_car, disk_car->rbuffer, count_new, offset_new / disk_car->sector_size * disk_car->sector_size);
...
static int
align_pwrite(int (*fnct_pread)(const disk_t *disk_car, void *buf, const unsigned int count, const uint64_t offset),
  ...
  if (count != count_new ||
    ...
    if (fnct_pread(disk_car, disk_car->wbuffer, count_new, offset_new / disk_car->sector_size * disk_car->sector_size) <
        0) {
    ...
    tmp =
        fnct_pwrite(disk_car, disk_car->wbuffer, count_new, offset_new / disk_car->sector_size * disk_car->sector_size);
