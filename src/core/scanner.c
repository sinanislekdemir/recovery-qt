...
static void indx_progress_cb(const char *msg, uint64_t current, uint64_t total, uint64_t found) {
...
static dir_partition_t scanner_init_fs(disk_t *disk, const partition_t *partition, dir_data_t *dir_data) {
...
static int is_valid_inode(const file_info_t *file, unsigned int depth, unsigned long int inode_known[]) {
  unsigned int i;
  for (i = 0; i < depth; i++) {
...
static int scan_dir_recursive(scan_tree_t *tree, disk_t *disk, const partition_t *partition, dir_data_t *dir_data,
                              const unsigned long int inode, const char *path, unsigned int depth,
  ...
  td_list_for_each(file_walker, &dir_list.list) {
    ...
    {
      const uint64_t num_sectors = fi->st_size > 0 ? (fi->st_size + disk->sector_size - 1) / disk->sector_size : 0;
      tree_add_path(tree, full_path, is_dir, fi->st_size, fi->st_ino, num_sectors, fi->td_mtime, disk->sector_size,
                    deleted);
    }
    ...
    if (is_dir && is_valid_inode(fi, depth + 1, inode_known)) {
      scan_dir_recursive(tree, disk, partition, dir_data, fi->st_ino, full_path, depth + 1, inode_known);
    }
  }
...
static int is_likely_fat_dir(const unsigned char *buffer, unsigned int size) {
  ...
  for (i = 0; i < 4 && (i + 1) * 32 <= size; i++) {
    ...
    if (de[i].attr == 0 && de[i].name[0] == ' ')
      return 0;
    if ((de[i].attr & ~(ATTR_RO | ATTR_HIDDEN | ATTR_SYS | ATTR_VOLUME | ATTR_DIR | ATTR_ARCH)) != 0)
      return 0;
    ...
    if (de[i].name[0] == '.' && (de[i].name[1] == ' ' || (de[i].name[1] == '.' && de[i].name[2] == ' ')))
      return 1;
  }
  return 1;
}

static int scanner_deep_fat(scan_tree_t *tree, disk_t *disk, const partition_t *partition) {
  ...
  if (disk->pread(disk, &fat_boot, sizeof(fat_boot), partition->part_offset) != (int)sizeof(fat_boot))
    return -1;
  ...
  fat_length = le16(fat_boot.fat_length) > 0 ? le16(fat_boot.fat_length) : le32(fat_boot.fat32_length);
  part_size = fat_sectors(&fat_boot) > 0 ? fat_sectors(&fat_boot) : le32(fat_boot.total_sect);
  start_data = (uint64_t)(le16(fat_boot.reserved) + fat_boot.fats * fat_length) * sector_size;
  no_of_cluster = ((uint64_t)part_size * sector_size - start_data) / cluster_size;

  fat_offset = partition->part_offset + (uint64_t)le16(fat_boot.reserved) * sector_size;
  fat_table = NULL;
  if (fat_length > 0 && fat_length <= (1024UL * 1024 * 1024 / sector_size)) {
    fat_table = (uint32_t *)MALLOC((size_t)(fat_length * sector_size));
  }
  if (fat_table) {
    if ((unsigned int)disk->pread(disk, fat_table, fat_length * sector_size, fat_offset) != fat_length * sector_size) {
      free(fat_table);
      fat_table = NULL;
    }
  }
  ...
  if (fat_table) {
    ...
    max_fat_clusters = (unsigned long int)fat_length * sector_size / sizeof(uint32_t);
  ...
  cluster = 2;
  if (g_scanner_resume_phase == SESSION_PHASE_FAT_DEEP && g_scanner_resume_offset > 2 &&
  ...
  for (; cluster < no_of_cluster + 2; cluster++) {
    ...
    if (fat_table) {
      ...
      next = get_next_cluster(disk, partition, partition->upart_type, le16(fat_boot.reserved), cluster);
      is_free = (next == 0);
    }
    ...
    if (is_free) {
      if (batch_buffer == NULL) {
        batch_count = 0;
        batch_buffer = (unsigned char *)MALLOC((size_t)FAT_BATCH_CLUSTERS * cluster_size);
      }
      ...
      if (batch_buffer && batch_count < FAT_BATCH_CLUSTERS) {
        ...
        offset = partition->part_offset + start_data + (uint64_t)(cluster - 2) * cluster_size;
        if ((unsigned int)disk->pread(disk, batch_buffer + buf_off, cluster_size, offset) == cluster_size) {
          batch_count++;
        }
      }
      ...
      for (b = 0; b < batch_count; b++) {
        ...
        if (is_likely_fat_dir(buf, cluster_size)) {
          ...
          td_list_for_each(file_walker, &dir_list.list) {
            ...
            snprintf(full_path, sizeof(full_path), "/Deep Scan Results/%s", fi->name);

            {
              const uint64_t num_sectors = fi->st_size > 0 ? (fi->st_size + sector_size - 1) / sector_size : 0;
              tree_add_path(tree, full_path, is_dir_de, fi->st_size, fi->st_ino, num_sectors, fi->td_mtime, sector_size,
                            1);
            }
  ...
  log_info("FAT deep scan: %lu free clusters, %lu found entries\n", (unsigned long)free_clusters,
           (unsigned long)g_deleted_count);

  return 0;
}

#if defined(HAVE_LIBEXT2FS)
static int scanner_deep_ext(scan_tree_t *tree, disk_t *disk, const partition_t *partition, dir_data_t *dir_data) {
  ...
  while (ext2fs_get_next_inode(scan, &ino, &inode) == 0) {
    ...
    scanned++;
    if (g_scanner_resume_phase == SESSION_PHASE_EXT_INODE && g_scanner_resume_offset > 0 &&
    ...
    if (scanned % 50000 == 0) {
      char msg[256];
      snprintf(msg, sizeof(msg), "EXT inode deep scan: %llu/%llu (%llu found)", scanned, total_inodes, found);
      update_progress(msg);
    }
    ...
    snprintf(full_path, sizeof(full_path), "/Deep Scan Results/inode_%lu.%s", (unsigned long)ino,
             is_dir ? "dir" : "file");

    tree_add_path(tree, full_path, is_dir, file_size, (uint64_t)ino, num_sectors, inode.i_mtime, sector_size, 1);
    ...
    if (!is_dir && file_size > 0) {
      ...
      if (retval == 0 && pblk != 0) {
        ...
        read_size = (unsigned int)(fs->blocksize < 4096 ? fs->blocksize : 4096);
        buf = (unsigned char *)MALLOC(read_size);
        if (buf) {
          const file_hint_t *hint;
          if (disk->pread(disk, buf, read_size, partition->part_offset + (uint64_t)pblk * fs->blocksize) ==
              (int)read_size) {
            hint = carver_check_header(buf, read_size, 0, NULL, 0);
            {
              ...
              if (fn) {
                if (hint && hint->extension) {
                  ...
                  dot = strrchr(fn->name, '.');
                  if (dot)
                    snprintf(new_name, sizeof(new_name), "%.*s.%s", (int)(dot - fn->name), fn->name, hint->extension);
                  else
                    snprintf(new_name, sizeof(new_name), "%s.%s", fn->name, hint->extension);
                  free(fn->name);
...
int scanner_run(scan_tree_t *tree, disk_t *disk, const partition_t *partition, bool deep) {
  ...
  scan_dir_recursive(tree, disk, partition, &dir_data, dir_data.current_inode, "/", 0, inode_known);
                      ...
                      1);
        {
          ...
          if (fn) {
            ...
            if (fn->cluster_list != NULL && fn->cluster_count > 0) {
              unsigned char *buf;
              unsigned int read_size;
              read_size = (unsigned int)(fn->cluster_size < 4096 ? fn->cluster_size : 4096);
              buf = (unsigned char *)MALLOC(read_size);
              if (buf) {
                const file_hint_t *hint;
                if (disk->pread(disk, buf, read_size, partition->part_offset + fn->cluster_list[0]) == (int)read_size) {
                  hint = carver_check_header(buf, read_size, 0, NULL, 0);
                  if (hint && hint->extension) {
                    char new_name[4096];
                    char *dot;
