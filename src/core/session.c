...
int session_save_checkpoint(uint64_t progress1, uint64_t progress2) {
  ...
  log_info("session_save_checkpoint: saving checkpoint (p1=%llu, p2=%llu)\n", (unsigned long long)progress1,
           (unsigned long long)progress2);

  return session_save(g_active_save_ctx->filepath, g_active_save_ctx->tree, g_active_save_ctx->disk,
                      g_active_save_ctx->partition, g_active_save_ctx->op_type, g_active_save_ctx->ext_filter,
                      progress1, progress2, g_active_save_ctx->resume_phase, g_active_save_ctx->resume_offset,
                      0 /* incomplete */);
}
...
static int session_write_node_rec(FILE *f, const file_node_t *node, const file_node_t *root, char *path_buf,
  ...
  if (fputc(flags, f) == EOF)
    return -1;
  if (write_le64(f, node->size) != 0)
    return -1;
  if (write_le64(f, node->first_sector) != 0)
    return -1;
  if (write_le64(f, node->num_sectors) != 0)
    return -1;
  if (write_le64(f, (uint64_t)node->mtime) != 0)
    return -1;
  if (write_le32(f, node->sector_size) != 0)
    return -1;
  if (write_le32(f, node->cluster_count) != 0)
    return -1;
  if (write_le32(f, node->cluster_size) != 0)
    return -1;
  ...
  if (node->type == NODE_DIR) {
    td_list_for_each(pos, &node->children) {
      ...
      if (session_write_node_rec(f, child, root, path_buf, path_buf_size) != 0)
        return -1;
    }
  }
...
static int session_write_tree(FILE *f, const scan_tree_t *tree) {
  ...
  td_list_for_each(pos, &tree->root->children) {
    file_node_t *child;
    child = td_list_entry(pos, file_node_t, siblings);
    if (session_write_node_rec(f, child, tree->root, path_buf, sizeof(path_buf)) != 0)
      return -1;
  }
...
static int session_read_tree(FILE *f, scan_tree_t *tree) {
  ...
  for (;;) {
    ...
    tree_add_path(tree, path_buf, is_dir, size, first_sector, num_sectors, mtime, sector_size, deleted);
    ...
    if (node) {
      ...
      if (cluster_count > 0 && !is_dir) {
        node->cluster_list = (uint64_t *)MALLOC(cluster_count * sizeof(uint64_t));
...
int session_save(const char *filepath, const scan_tree_t *tree, const disk_t *disk, const partition_t *partition,
  ...
  if (fwrite("RECOVRY_SES", 1, 12, f) != 12) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, SESSION_VERSION) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, (uint32_t)flags) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, (uint32_t)op_type) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, (uint64_t)time(NULL)) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, progress1) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, progress2) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, (uint32_t)resume_phase) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, resume_offset) != 0) {
    fclose(f);
    return -1;
  }

  if (write_le32(f, disk->sector_size) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, disk->disk_size) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, partition->part_offset) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, partition->part_size) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, (uint32_t)partition->upart_type) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, partition->part_type_i386) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le32(f, (flags & SESSION_FLAG_LUKS_DECRYPTED) ? 1 : 0) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le64(f, (flags & SESSION_FLAG_LUKS_DECRYPTED) ? partition->part_offset : 0) != 0) {
    fclose(f);
    return -1;
  }
  ...
  if (write_le16(f, dev_len) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le16(f, model_len) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le16(f, filter_len) != 0) {
    fclose(f);
    return -1;
  }
  if (write_le16(f, 0) != 0) {
  ...
  log_info("session_save: %s saved (%s)\n", filepath, (flags & SESSION_FLAG_COMPLETED) ? "completed" : "in progress");
  return 0;
}
