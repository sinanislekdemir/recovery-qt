  ...
  while (1) {
    ...
    if (check_command(current_cmd, "s,", 2) == 0) {
      ...
      new_partition->part_offset =
          (uint64_t)ask_number_cli(current_cmd, new_partition->part_offset / disk_car->sector_size,
...
static int test_structure_xbox(const list_part_t *list_part) { /* Return 1 if bad*/
