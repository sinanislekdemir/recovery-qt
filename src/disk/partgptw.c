...
static void partition_generate_gpt_entry(struct gpt_ent *gpt_entry, const partition_t *partition,
                                         const disk_t *disk_car, const struct gpt_ent *gpt_entries_org) {
...
static int write_part_gpt_i386(disk_t *disk_car, const list_part_t *list_part) {
  ...
  if (part_mac != NULL && (part_linux != NULL || part_windows != NULL)) { /* For bootcamp, the layout should be
     * 1 EFI
     * 2 MacOS X
     * 3 Linux if any
     * 4 Windows
  ...
  } else { /* The Protective MBR contains one partition entry of OS type 0xEE and
...
int write_part_gpt(disk_t *disk_car, const list_part_t *list_part, const int ro, const int verbose) {
  ...
  for (element = list_part; element != NULL; element = element->next) {
    if (element->part->order > 0 && element->part->order <= hdr_entries) {
      partition_generate_gpt_entry(&gpt_entries[element->part->order - 1], element->part, disk_car, gpt_entries_org);
    }
  }
  gpt = (struct gpt_hdr *)MALLOC(disk_car->sector_size);
  gpt_org = (struct gpt_hdr *)MALLOC(disk_car->sector_size);
  if (disk_car->pread(disk_car, gpt_org, disk_car->sector_size, disk_car->sector_size) == (int)disk_car->sector_size)
  ...
  if ((unsigned)disk_car->pwrite(disk_car, gpt_entries, gpt_entries_size,
                                 le64(gpt->hdr_lba_table) * disk_car->sector_size) != gpt_entries_size) {
    free(gpt_org);
  ...
  if ((unsigned)disk_car->pwrite(disk_car, gpt, disk_car->sector_size,
                                 le64(gpt->hdr_lba_self) * disk_car->sector_size) != disk_car->sector_size) {
    free(gpt_org);
    free(gpt_entries_org);
    free(gpt);
    free(gpt_entries);
