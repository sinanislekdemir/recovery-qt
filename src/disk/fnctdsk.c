...
uint64_t CHS2offset(const disk_t *disk_car, const CHS_t *CHS) {
  return (((uint64_t)CHS->cylinder * disk_car->geom.heads_per_cylinder + CHS->head) * disk_car->geom.sectors_per_head +
          CHS->sector - 1) *
...
list_part_t *insert_new_partition(list_part_t *list_part, partition_t *part, const int force_insert,
  ...
  *insert_error = 0;
  ...
  for (next = list_part;; next = next->next) { /* prev new next */
    ...
    if ((next == NULL) || (part->part_offset < next->part->part_offset) ||
      ...
      if (force_insert == 0 && (next != NULL) && (next->part->part_offset == part->part_offset) &&
          ...
          (next->part->part_type_mac == part->part_type_mac) && (next->part->part_type_sun == part->part_type_sun) &&
          (next->part->part_type_xbox == part->part_type_xbox) &&
          (next->part->upart_type == part->upart_type || part->upart_type == UP_UNK)) { /*CGR 2004/05/31*/
