...
static list_part_t *read_part_gpt_aux(disk_t *disk_car, const int verbose, const int saveheader,
                                      const uint64_t hdr_lba) {
  struct gpt_hdr *gpt;
  ...
  if (hdr_lba == 1) {
    if ((uint64_t)le64(gpt->hdr_lba_self) + le32(gpt->hdr_size) - 1 >= gpt_entries_offset ||
      ...
      screen_buffer_add("GPT: The primary GUID Partition Entry array must be located after the primary GUID Partition "
                        "Table Header and end before the FirstUsableLBA.\n");
      free(gpt);
      return NULL;
    }
  }
...
list_part_t *add_partition_gpt_cli(const disk_t *disk_car, list_part_t *list_part, char **current_cmd) {
  ...
  while (1) {
    ...
    if (check_command(current_cmd, "s,", 2) == 0) {
      ...
      new_partition->part_size =
          (uint64_t)ask_number_cli(current_cmd,
...
static int test_structure_gpt(const list_part_t *list_part) { /* Return 1 if bad*/
  int res;
