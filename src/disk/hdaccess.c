...
static unsigned int disk_get_sector_size(const int hd_h, const char *device, const int verbose) {
...
#ifdef DIOCGDINFO
  {
    ...
    if (ioctl(hd_h, DIOCGDINFO, &geometry) == 0) { /* I can get the geometry */
...
static void disk_get_geometry(CHSgeometry_t *geom, const int hd_h, const char *device, const int verbose) {
  ...
  if (check_geometry(geom) < 0) {
    ...
    if (ioctl(hd_h, HDIO_GETGEO_BIG, &geometry) >= 0) { /* I can get the geometry */
      if (verbose > 1) {
        log_verbose("disk_get_geometry HDIO_GETGEO_BIG %s Ok (%u,%u,%u)\n", device, geometry.cylinders, geometry.heads,
                    geometry.sectors);
      }
  ...
  if (check_geometry(geom) < 0) {
    ...
    if (ioctl(hd_h, HDIO_GETGEO, &geometry) >= 0) { /* I can get the geometry */
      if (verbose > 1) {
...
void update_disk_car_fields(disk_t *disk_car) {
  if (disk_car->disk_real_size == 0) {
    ...
    const unsigned long int cylinder_num = disk_car->disk_real_size / (uint64_t)disk_car->geom.heads_per_cylinder /
                                           (uint64_t)disk_car->geom.sectors_per_head / (uint64_t)disk_car->sector_size;
...
static int file_pread_aux(const disk_t *disk, void *buf, const unsigned int count, const uint64_t offset) {
  ...
  if (lseek(fd, offset, SEEK_SET) < 0) {
    log_error("file_pread(%d,%u,buffer,%lu(%u/%u/%u)) lseek err %s\n", fd, (unsigned)(count / disk->sector_size),
              (long unsigned int)(offset / disk->sector_size), offset2cylinder(disk, offset), offset2head(disk, offset),
              offset2sector(disk, offset), strerror(errno));
    return -1;
  }
  {
  ...
  if (ret != count) {
    if (offset + count <= disk->disk_size && offset + count <= disk->disk_real_size) {
      log_error("file_pread(%d,%u,buffer,%lu(%u/%u/%u)) read err: ", fd, (unsigned)(count / disk->sector_size),
                (long unsigned)(offset / disk->sector_size), offset2cylinder(disk, offset), offset2head(disk, offset),
