    ...
    for (i = 0; i < 4; i++) {
      ...
      if (p->sys_ind != 0) {
        ...
        if (is_extended(p->sys_ind)) {
          i386_entry2partition(disk_car, partition_main_ext->part_offset, new_partition, p, STATUS_EXT_IN_EXT, order,
                               verbose, saveheader);
          aff_part_buffer(AFF_PART_ORDER | AFF_PART_STATUS, disk_car, new_partition);
          if (new_partition->errcode != BAD_NOERR) {
            screen_buffer_add("%s\n", errmsg_i386_entry2partition(new_partition->errcode));
          }
          {
            if ((new_partition->part_offset <= partition_main_ext->part_offset) ||
                (new_partition->part_offset + new_partition->part_size - 1 >
                 partition_main_ext->part_offset + partition_main_ext->part_size -
                     1)) { /* Must be IN partition_main_ext */
              ...
              for (element = list_part; element != NULL; element = element->next) {
                ...
                if (partition->status == STATUS_EXT_IN_EXT) {
                  if (((partition->part_offset >= new_partition->part_offset) &&
                       (partition->part_offset <= new_partition->part_offset + new_partition->part_size - 1)) ||
                      ((partition->part_offset + partition->part_size - 1 >= new_partition->part_offset) &&
                       (partition->part_offset + partition->part_size - 1 <=
                        new_partition->part_offset + partition->part_size -
                            1))) { /* New Partition start or end mustn't been in partition */
          ...
          i386_entry2partition(disk_car, partition_ext->part_offset, new_partition, p, STATUS_LOG, order, verbose,
                               saveheader);
          order++;
          if (verbose > 1)
            log_dos_entry(p);
...
static int test_MBR_over(const disk_t *disk_car, const list_part_t *list_part) { /* Test if partitions overlap */
  int res = 0;
  const list_part_t *element;
  for (element = list_part; element != NULL; element = element->next)
  ...
  for (element = list_part; element != NULL; element = element->next) {
    ...
    case STATUS_EXT:
      if ((element->part->order >= 1) && (element->part->order <= 4)) {
        partition2_i386_entry(disk_car, (uint64_t)0, element->part, pt_offset(buffer, element->part->order - 1));
      }
      break;
    case STATUS_LOG:
      break;
...
static int write_all_log_i386(disk_t *disk_car, const list_part_t *list_part, const int ro, const int verbose) {
  ...
  if (pos_ext->next == NULL || (pos_ext->next->part->status != STATUS_LOG)) {
    ...
    for (element = pos_ext->next; (element != NULL) && (element->part->status == STATUS_LOG); element = element->next) {
      ...
      if (element->next != NULL &&
          (element->next->part->status == STATUS_LOG)) { /* Construit le pointeur vers la prochaine partition logique */
        CHS_t nextext_start;
        ...
        bloc_nextext->part_size =
            (uint64_t)element->next->part->part_offset + element->next->part->part_size - bloc_nextext->part_offset;
...
static int write_MBR_code_i386_aux(unsigned char *buffer) {
  ...
  const unsigned char mbr_code_testdisk[DEFAULT_SECTOR_SIZE] = {
      , 'T',  'e',  's',  't',  'D',  'i',  's',  'k',  0x0d, 0x0a, 0x10, 0x00,
      0x01, 0x00, 0x00, 0x7c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, '1',  '2',  '3',  '4',  'F',
      0x00, 0x00, 0x41, 'N',  'D',  'T',  'm',  'b',  'r',  0x00, 0x02, 0x02, 0x02, 0x1f, 0xc7, 0x00, 0x00, 0x80, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xa5, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void partition2_i386_entry(const disk_t *disk_car, const uint64_t pos, const partition_t *partition,
  ...
  if (start.cylinder > 1023) { /* Partition Magic 5 uses CHS=(1023,0,1) if extended or last logical *
...
static int i386_entry2partition(disk_t *disk_car, const uint64_t offset, partition_t *partition,
  ...
  if (((end_calculated.cylinder <= 1023) && (C_H_S2offset(disk_car, end.cylinder, end.head, end.sector) !=
...
int parti386_can_be_ext(const disk_t *disk_car, const partition_t *partition) {
  return ((offset2head(disk_car, partition->part_offset) > 0) &&
          (offset2cylinder(disk_car, partition->part_offset) != 0 ||
           offset2head(disk_car, partition->part_offset) != 1 || offset2sector(disk_car, partition->part_offset) != 1));
}

static int test_structure_i386(const list_part_t *list_part) { /* Return 1 if bad*/
...
list_part_t *add_partition_i386_cli(disk_t *disk_car, list_part_t *list_part, char **current_cmd) {
  ...
  while (1) {
    ...
    if (check_command(current_cmd, "c,", 2) == 0) {
      start.cylinder =
          ask_number_cli(current_cmd, start.cylinder, 0, disk_car->geom.cylinders - 1, "Enter the starting cylinder ");
    } else if (check_command(current_cmd, "h,", 2) == 0) {
      start.head =
          ask_number_cli(current_cmd, start.head, 0, disk_car->geom.heads_per_cylinder - 1, "Enter the starting head ");
    } else if (check_command(current_cmd, "s,", 2) == 0) {
      start.sector =
          ask_number_cli(current_cmd, start.sector, 1, disk_car->geom.sectors_per_head, "Enter the starting sector ");
          ...
          ask_number_cli(current_cmd, end.head, 0, disk_car->geom.heads_per_cylinder - 1, "Enter the ending head ");
    } else if (check_command(current_cmd, "S,", 2) == 0) {
      end.sector =
          ask_number_cli(current_cmd, end.sector, 1, disk_car->geom.sectors_per_head - 1, "Enter the ending sector ");
      ...
      if (test_structure_i386(list_part) == 0) { /* Check if the partition can be Logical, Bootable or Primary */
...
static void init_structure_i386(const disk_t *disk_car, list_part_t *list_part, const int verbose) {
  ...
  if (vista_partition > 0 || test_structure_i386(new_list_part)) { /* Handle Vista partition */
    unsigned int i;
    int set_prim_bootable_done = 0;
