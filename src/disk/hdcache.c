...
static int cache_pread_aux(disk_t *disk_car, void *buffer, const unsigned int count, const uint64_t offset,
...
#endif
  {
    ...
    for (i = 0, cache_buffer_nbr = data->cache_buffer_nbr; i < CACHE_BUFFER_NBR;
      ...
      if (cache->cache_offset <= offset && offset < cache->cache_offset + cache->cache_size && cache->buffer != NULL &&
          cache->cache_size > 0) {
        ...
        if (count <= data_available) {
          ...
          log_info("cache USE %5u count=%u, coffset=%llu, ctstatus=%d, call again cache_pread_aux\n", cache_buffer_nbr,
                   cache->cache_size, (long long unsigned)cache->cache_offset, cache->cache_status);
          data->nbr_fnct_sect += data_available;
#endif
          memcpy(buffer, cache->buffer + offset - cache->cache_offset, data_available);
          return res + cache_pread_aux(disk_car, (unsigned char *)buffer + data_available, count - data_available,
  ...
  }
  {
    ...
    if (cache->buffer_size < count_new) { /* Buffer is too small, drop it */
      free(cache->buffer);
      cache->buffer = NULL;
    }
    if (cache->buffer == NULL) { /* Allocate buffer */
    ...
    /* split the read sector by sector */
    {
      ...
      for (off = 0; off < count; off += disk_car->sector_size) {
        if (cache_pread_aux(disk_car, (unsigned char *)buffer + off,
                            (disk_car->sector_size < count - off ? disk_car->sector_size : count - off), offset + off,
                            0) <= 0) {
          return off;
        }
      }
...
static void cache_clean(disk_t *disk_car) {
  if (disk_car->data) {
    ...
    log_info("%s\ncache_pread total_call=%u, total_count=%llu\n      read total_call=%u, total_count=%llu\n",
             data->disk_car->description(data->disk_car), data->nbr_fnct_call, (long long unsigned)data->nbr_fnct_sect,
