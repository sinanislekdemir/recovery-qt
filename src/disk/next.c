...
static inline uint64_t CHS_to_offset(const unsigned int C, const int H, const int S, const disk_t *disk_car) {
  return (((uint64_t)C * disk_car->geom.heads_per_cylinder + H) * disk_car->geom.sectors_per_head +
          (S > 0 ? S - 1 : S)) *
...
static void update_location(void) {
  ...
  for (i = 0; i < search_location_nbr; i++) {
    ...
    if (cur->offset == src->offset && cur->inc >= src->inc && cur->inc % src->inc == 0) {
      cur->inc = src->inc;
      return;
    }
    if (cur->inc == 0) {
      ...
      if (cur->offset == src->offset && src->inc >= cur->inc && src->inc % cur->inc == 0)
        return;
      if (cur->inc == src->inc && cur->offset >= src->offset && (cur->offset - src->offset) % cur->inc == 0) {
        cur->offset = src->offset;
        return;
      }
      if (cur->inc == src->inc && src->offset >= cur->offset && (src->offset - cur->offset) % cur->inc == 0)
        return;
    }
  }
  if (search_location_nbr < SEARCH_LOCATION_MAX)
