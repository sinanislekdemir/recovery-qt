...
DiskSelectionWidget::DiskSelectionWidget(QWidget *parent)
    : QWidget(parent), m_titleLabel(nullptr), m_tableView(nullptr), m_model(nullptr), m_refreshBtn(nullptr),
      m_openImageBtn(nullptr), m_continueBtn(nullptr), m_proceedBtn(nullptr), m_quitBtn(nullptr) {
  setupUi();
  applyTheme();
  refreshDisks();
}
...
void DiskSelectionWidget::setupUi() {
  ...
  m_openImageBtn->setStyleSheet("QPushButton {"
  ...
  m_continueBtn->setStyleSheet("QPushButton {"
                               "  background-color: #4C566A;"
                               "  color: #ECEFF4;"
                               "  border: 2px solid #81A1C1;"
                               "  padding: 6px 16px;"
  ...
  connect(m_tableView->selectionModel(), &QItemSelectionModel::selectionChanged, this,
          &DiskSelectionWidget::onSelectionChanged);
}

void DiskSelectionWidget::applyTheme() {
  setStyleSheet(QStringLiteral("DiskSelectionWidget { background-color: #2E3440; }") + Theme::globalStyleSheet());
}
...
void DiskSelectionWidget::onOpenImage() {
  QString path = QFileDialog::getOpenFileName(this, tr("Open Disk Image"), QString(),
                                              tr("Disk Images (*.img *.dd *.raw *.dsk *.vhd *.vmdk *.vdi *.e01 *.s01 "
                                                 "*.aff *.hdd *.qcow *.qcow2 *.bin *.iso);;All Files (*)"));
  if (path.isEmpty())
    return;

  Disk img = Disk::openDevice(path, TESTDISK_O_RDONLY);
  if (!img.isValid()) {
    QMessageBox::warning(this, tr("Open Failed"), tr("Could not open or read the disk image:\n%1").arg(path));
    return;
  }
