...
DeletedFileFilterProxy::DeletedFileFilterProxy(QObject *parent)
    : QSortFilterProxyModel(parent), m_deletedOnly(false)
{
}
...
{
    setStyleSheet(QStringLiteral("BrowserWidget { background-color: #2E3440; }")
...
void BrowserWidget::setFileModel(FileTreeModel *model)
{
    ...
    if (model)
        connect(model, &FileTreeModel::markedChanged,
                this, &BrowserWidget::onMarkedChanged);
}
...
{
    m_statusBar->setText(tr("Marked: %1 files (%2)")
        .arg(count)
