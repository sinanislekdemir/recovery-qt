...
ImagePreviewDialog::ImagePreviewDialog(QWidget *parent)
    : QDialog(parent), m_imageLabel(nullptr), m_infoLabel(nullptr), m_toolbar(nullptr), m_zoomInAction(nullptr),
      m_zoomOutAction(nullptr), m_origAction(nullptr), m_fitAction(nullptr), m_scroll(nullptr), m_zoomFactor(1.0),
      m_fitToWindow(true) {
  ...
  m_toolbar->setStyleSheet("QToolBar { background-color: #3B4252; border: none; spacing: 4px; padding: 2px 4px; }");
  ...
  m_scroll->setStyleSheet("QScrollArea { background-color: #2E3440; border: none; }");
...
bool ImagePreviewDialog::loadFromData(const QByteArray &data) {
  ...
  m_infoLabel->setText(QString("%1 x %2  |  100%").arg(m_originalPixmap.width()).arg(m_originalPixmap.height()));
...
void ImagePreviewDialog::applyZoom() {
  ...
  if (m_fitToWindow) {
    ...
    if (viewSize.isEmpty())
      return;
    scaled = m_originalPixmap.scaled(viewSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    m_zoomFactor = (double)scaled.width() / m_originalPixmap.width();
  } else {
    QSize sz = m_originalPixmap.size() * m_zoomFactor;
    scaled = m_originalPixmap.scaled(sz, Qt::KeepAspectRatio, Qt::SmoothTransformation);
  }
