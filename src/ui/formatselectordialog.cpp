...
FormatSelectorDialog::FormatSelectorDialog(QWidget *parent)
    : QDialog(parent), m_filterEdit(nullptr), m_listWidget(nullptr), m_selectAllBtn(nullptr), m_deselectAllBtn(nullptr),
      m_okBtn(nullptr), m_photoBtn(nullptr), m_docBtn(nullptr), m_archiveBtn(nullptr), m_audioBtn(nullptr) {
  setupUi();
  applyTheme();
}
...
void FormatSelectorDialog::applyTheme() {
  setStyleSheet(QStringLiteral("FormatSelectorDialog { background-color: #2E3440; }") + Theme::globalStyleSheet() +
                Theme::listWidgetStyle() + "QPushButton:default { border: 2px solid #88C0D0; }");
}

void FormatSelectorDialog::setExtensions(const QVector<SignatureInfo> &sigs) {
  m_signatures = sigs;
  std::sort(m_signatures.begin(), m_signatures.end(),
            [](const SignatureInfo &a, const SignatureInfo &b) { return a.priority < b.priority; });
...
static void selectFormats(const QSet<QString> &exts, QListWidget *list) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!item)
      continue;
    item->setCheckState(exts.contains(item->data(Qt::UserRole).toString()) ? Qt::Checked : Qt::Unchecked);
  }
}

void FormatSelectorDialog::onQuickPhoto() {
  QSet<QString> exts = {"jpg", "jpeg", "mov",  "png", "gif", "bmp", "avi",  "mp4", "mkv",  "webp",
                        "svg", "mts",  "m2ts", "3gp", "wmv", "flv", "ico",  "tif", "tiff", "psd",
                        "cr2", "nef",  "orf",  "dng", "raw", "arw", "heic", "jp2"};
  selectFormats(exts, m_listWidget);
}

void FormatSelectorDialog::onQuickDocuments() {
  QSet<QString> exts = {"pdf", "doc",  "docx", "xls",  "xlsx", "ppt", "pptx", "txt",    "rtf",
                        "csv", "html", "xml",  "json", "odt",  "ods", "odp",  "parquet"};
  selectFormats(exts, m_listWidget);
}

void FormatSelectorDialog::onQuickArchives() {
  QSet<QString> exts = {"zip", "rar", "7z", "tar", "gz", "bz2", "xz", "lz", "lzh", "cab", "iso", "arj", "ace"};
  selectFormats(exts, m_listWidget);
}

void FormatSelectorDialog::onQuickAudio() {
  QSet<QString> exts = {"mp3", "wav", "flac", "ogg", "wma", "m4a", "aac", "aiff", "au", "mid", "ape", "wv"};
  selectFormats(exts, m_listWidget);
}
