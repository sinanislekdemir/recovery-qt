...
ProgressDialog::ProgressDialog(QWidget *parent)
    : QDialog(parent), m_titleLabel(nullptr), m_progressBar(nullptr), m_statusLabel(nullptr), m_fileNameLabel(nullptr),
      m_detailLabel(nullptr), m_cancelBtn(nullptr), m_finished(false) {
...
void ProgressDialog::applyTheme() {
  setStyleSheet(QStringLiteral("ProgressDialog { background-color: #2E3440; }") + Theme::globalStyleSheet());
}
...
void ProgressDialog::setFinished(bool ok, const QString &summary) {
  ...
  if (ok) {
    m_progressBar->setStyleSheet("QProgressBar { border: 1px solid #4C566A; background: #3B4252;"
