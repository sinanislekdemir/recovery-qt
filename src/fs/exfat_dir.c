...
static int dir_exfat_aux(const unsigned char *buffer, const unsigned int size, const dir_data_t *dir_data,
  ...
  for (offset = 0; offset < size; offset += 0x20) {
    if ((buffer[offset] & 0x80) == 0 && (dir_data->param & FLAG_LIST_DELETED) != FLAG_LIST_DELETED)
      continue;
    if ((buffer[offset] & 0x7f) == 0x05) { /* File directory entry */
  ...
  while (!is_EOC(cluster) && cluster >= 2 && nbr_cluster < NBR_CLUSTER_MAX && stop == 0) {
    ...
    if (stop == 0) {
      ...
      if (exfat_meth == exFAT_NEXT_CLUSTER)
        cluster++;
      else if (exfat_meth == exFAT_NEXT_FREE_CLUSTER) { /* Deleted directories are composed of "free" clusters */
#if 0
...
static copy_file_t exfat_copy(disk_t *disk, const partition_t *partition, dir_data_t *dir_data,
  ...
  while (cluster >= 2 && cluster <= total_clusters && file_size > 0) {
    ...
    if (file_size > 0) {
      ...
      if (exfat_meth == exFAT_NEXT_CLUSTER)
        cluster++;
      else if (exfat_meth == exFAT_NEXT_FREE_CLUSTER) { /* Deleted file are composed of "free" clusters */
        while (++cluster < total_clusters && exfat_get_next_cluster(disk, partition, start_exfat1, cluster) != 0)
          ;
      }
    }
  }
