    ...
    if (bsd_header->d_partitions[i].p_fstype > 0) {
      if (verbose > 0) {
        ...
        log_CHS_from_LBA(disk_car,
                         le32(bsd_header->d_partitions[i].p_offset) + le32(bsd_header->d_partitions[i].p_size) - 1);
        log_info("\n");
      }
    }
  }
...
int recover_BSD(const disk_t *disk_car, const struct disklabel *bsd_header, partition_t *partition, const int verbose,
  ...
  if (test_BSD(disk_car, bsd_header, partition, verbose, dump_ind, BSD_MAXPARTITIONS) == 0) {
    ...
    for (i = 0; i < BSD_MAXPARTITIONS; i++) {
      if (bsd_header->d_partitions[i].p_fstype > 0) {
        if (i_max_p_offset == -1 ||
            le32(bsd_header->d_partitions[i].p_offset) > le32(bsd_header->d_partitions[i_max_p_offset].p_offset))
          i_max_p_offset = i;
      }
    }
    if (i_max_p_offset >= 0)
