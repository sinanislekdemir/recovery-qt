...
static int pbkdf2(const EVP_MD *md, const char *pass, size_t passlen, const unsigned char *salt, size_t saltlen,
                  unsigned int iter, unsigned char *out, size_t outlen) {
  return PKCS5_PBKDF2_HMAC(pass, (int)passlen, salt, (int)saltlen, (int)iter, md, (int)outlen, out) == 1 ? 0 : -1;
}
...
static int luks_parse_cipher(const char *cipher, const char *mode, luks_cipher_t *out) {
  const char *dash;
  const char *iv;
  char chain[16];
  size_t clen;
...
static int hash_once(const EVP_MD *md, const unsigned char *in, size_t inlen, unsigned char *out,
                     unsigned int *outlen) {
  EVP_MD_CTX *ctx = EVP_MD_CTX_new();
  int ok;
  if (ctx == NULL)
    return -1;
  ok = EVP_DigestInit_ex(ctx, md, NULL) == 1 && EVP_DigestUpdate(ctx, in, inlen) == 1 &&
       EVP_DigestFinal_ex(ctx, out, outlen) == 1;
  EVP_MD_CTX_free(ctx);
  return ok ? 0 : -1;
}

static int aes_ecb_encrypt_block(const unsigned char *key, size_t keylen, const unsigned char *in, unsigned char *out) {
  ...
  if (EVP_EncryptInit_ex(ctx, cipher, NULL, key, NULL) != 1 || EVP_EncryptUpdate(ctx, out, &outl, in, 16) != 1) {
 ...
 */
static int luks_decrypt_region(const luks_cipher_t *c, const unsigned char *key, size_t keylen,
                               unsigned int sector_size, uint64_t iv_base, const unsigned char *in, unsigned char *out,
                               size_t len) {
  const EVP_CIPHER *cipher;
  size_t off;
...
static int af_diffuse(const EVP_MD *md, unsigned char *src, size_t size) {
  ...
  for (i = 0; i < nblocks; i++) {
    ...
    if (ctx == NULL)
      return -1;
    if (EVP_DigestInit_ex(ctx, md, NULL) != 1 || EVP_DigestUpdate(ctx, iv, 4) != 1 ||
  ...
  if (padding > 0) {
    ...
    if (ctx == NULL)
      return -1;
    if (EVP_DigestInit_ex(ctx, md, NULL) != 1 || EVP_DigestUpdate(ctx, iv, 4) != 1 ||
        EVP_DigestUpdate(ctx, src + nblocks * digest_size, padding) != 1 || EVP_DigestFinal_ex(ctx, buf, &dlen) != 1) {
      EVP_MD_CTX_free(ctx);
      return -1;
    }
    EVP_MD_CTX_free(ctx);
...
static int af_merge(const EVP_MD *md, const unsigned char *src, unsigned char *dst, size_t blocksize,
...
static const char *luksdec_description(disk_t *disk) {
  snprintf(disk->description_txt, sizeof(disk->description_txt), "LUKS decrypted %s", disk->device ? disk->device : "");
...
static int luksdec_pread(disk_t *disk, void *buf, const unsigned int count, const uint64_t offset) {
  ...
  if (luks_decrypt_region(&d->cipher, d->mk, d->mk_len, ss, iv_base, cbuf, pbuf, clen) != 0) {
    free(cbuf);
    free(pbuf);
    return -1;
  }
...
static int luksdec_nopwrite(disk_t *disk, const void *buf, const unsigned int count, const uint64_t offset) {
  (void)disk;
  (void)buf;
  (void)offset;
  return (int)count;
...
static disk_t *luksdec_build_disk(disk_t *base, const unsigned char *mk, size_t mk_len, uint64_t payload_offset,
                                  unsigned int enc_sector_size, uint64_t iv_tweak, const luks_cipher_t *cipher) {
  disk_t *disk;
  ...
  payload_size = base->disk_size > payload_offset ? base->disk_size - payload_offset : 0;
...
static disk_t *luks1_unlock(disk_t *base, uint64_t part_offset, const unsigned char *hdr_sector,
  ...
  for (i = 0; i < LUKS_NUMKEYS && result == NULL; i++) {
    ...
    if (pbkdf2(md, passphrase, strlen(passphrase), ks->passwordSalt, LUKS_SALTSIZE, iterations, derived, key_bytes) !=
        0)
      continue;
    ...
    if (base_read(base, km, part_offset + (uint64_t)km_offset * LUKS1_SECTOR_SIZE, km_len) != 0) {
      free(km);
      free(split);
      continue;
    }
    ...
    if (luks_decrypt_region(&cipher, derived, key_bytes, LUKS1_SECTOR_SIZE, 0, km, split, km_len) != 0) {
      free(km);
      free(split);
      continue;
    }
    ...
    if (pbkdf2(md, (const char *)candidate, key_bytes, hdr->mkDigestSalt, LUKS_SALTSIZE, be32(hdr->mkDigestIterations),
    ...
    if (memcmp(check, hdr->mkDigest, LUKS_DIGESTSIZE) == 0) {
      ...
      result = luksdec_build_disk(base, candidate, key_bytes,
                                  part_offset + (uint64_t)payload_off_sectors * LUKS1_SECTOR_SIZE, LUKS1_SECTOR_SIZE, 0,
                                  &cipher);
    }
...
static int luks2_derive_key(const jnode_t *kdf, const char *passphrase, unsigned char *out, size_t outlen) {
  ...
  if (strcmp(type, "pbkdf2") == 0) {
    ...
    if (md == NULL)
      return -1;
    return pbkdf2(md, passphrase, strlen(passphrase), salt, saltlen, iter, out, outlen);
  }
  if (strcmp(type, "argon2i") == 0 || strcmp(type, "argon2id") == 0) {
    ...
    if (strcmp(type, "argon2id") == 0)
      rc = argon2id_hash_raw(t_cost, m_cost, par, passphrase, strlen(passphrase), salt, saltlen, out, outlen);
    else
      rc = argon2i_hash_raw(t_cost, m_cost, par, passphrase, strlen(passphrase), salt, saltlen, out, outlen);
...
static const jnode_t *luks2_find_digest(const jnode_t *digests, const char *keyslot_id) {
  const jnode_t *dg;
  if (digests == NULL)
    return NULL;
  for (dg = digests->child; dg != NULL; dg = dg->next) {
    ...
    for (e = kslist->child; e != NULL; e = e->next)
      if (e->type == J_STR && e->str != NULL && strcmp(e->str, keyslot_id) == 0)
        return dg;
  }
  return NULL;
}

static int luks2_verify_digest(const jnode_t *digest, const unsigned char *mk, size_t mk_len) {
  ...
  if (saltlen < 0 || diglen < 0)
    return -1;
  if (pbkdf2(md, (const char *)mk, mk_len, salt, saltlen, iter, got, (size_t)diglen) != 0)
    return -1;
  return memcmp(got, want, diglen) == 0 ? 0 : -1;
}

static disk_t *luks2_unlock(disk_t *base, uint64_t part_offset, const unsigned char *bin_hdr, const char *passphrase) {
  uint64_t hdr_size;
  char *json_text = NULL;
  size_t json_len;
  jnode_t *root = NULL;
  ...
  for (ks = keyslots->child; ks != NULL && result == NULL; ks = ks->next) {
    ...
    log_info("luks2_unlock: keyslot key='%s' key_size=%llu stripes=%u\n", ks->key ? ks->key : "?",
             (unsigned long long)key_size, stripes);
    ...
    if (luks_decrypt_region(&area_cipher, derived, (size_t)area_key_size, LUKS1_SECTOR_SIZE, 0, km, split, km_len) !=
        0) {
      free(km);
      free(split);
      continue;
    ...
    if (luks2_verify_digest(digest, candidate, (size_t)key_size) == 0) {
      ...
      const jnode_t *seg;
      if (seglist != NULL && seglist->type == J_ARR && seglist->child != NULL && seglist->child->type == J_STR)
      ...
      if (seg != NULL) {
        ...
        if (luks_parse_cipher_spec(seg_enc, &seg_cipher) == 0 && luks_valid_keylen(seg_cipher.chain, key_size)) {
          log_info("luks2_unlock: matched; segment %s offset=%llu ss=%u\n", seg_enc, (unsigned long long)seg_off,
                   seg_ss);
          result = luksdec_build_disk(base, candidate, (size_t)key_size, part_offset + seg_off, seg_ss, iv_tweak,
                                      &seg_cipher);
        } else {
