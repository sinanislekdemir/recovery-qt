...
int search_NTFS_backup(unsigned char *buffer, disk_t *disk, partition_t *partition, const int verbose,
    ...
    return -1;
  {
    ...
    if (le16(ntfs_header->marker) == 0xAA55 && recover_NTFS(disk, ntfs_header, partition, verbose, dump_ind, 1) == 0)
      return 1;
#endif
  }
  return 0;
...
int search_type_0(const unsigned char *buffer, disk_t *disk, partition_t *partition, const int verbose,
  ...
  if (memcmp(fatx_block->magic, "FATX", 4) == 0 && recover_FATX(fatx_block, partition) == 0)
    return 1;
  if (memcmp(luks->magic, LUKS_MAGIC, LUKS_MAGIC_L) == 0 && recover_LUKS(disk, luks, partition, verbose, dump_ind) == 0)
    return 1;
...
int search_type_16(unsigned char *buffer, disk_t *disk, partition_t *partition, const int verbose, const int dump_ind) {
...
#if !defined(DISABLED_FOR_FRAMAC)
  {
    ...
    /* Test UFS */
    if ((le32(ufs->fs_magic) == UFS_MAGIC || be32(ufs->fs_magic) == UFS_MAGIC || le32(ufs->fs_magic) == UFS2_MAGIC ||
         be32(ufs->fs_magic) == UFS2_MAGIC) &&
...
int search_type_128(unsigned char *buffer, disk_t *disk, partition_t *partition, const int verbose,
                    const int dump_ind) {
  if (verbose > 2) {
...
int search_type_2048(unsigned char *buffer, disk_t *disk, partition_t *partition, const int verbose,
                     const int dump_ind) {
  if (verbose > 2) {
