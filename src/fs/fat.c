  ...
  if ((fat_header->sectors_per_cluster > 0) && (fat_header->sectors_per_cluster <= 128)) {
    ...
    if ((unsigned)disk_car->pread(
      ...
      for (i = 0; (i < 16 * fat_header->sectors_per_cluster) && (stop == 0);
           i++) { /* Test attribut volume name and check if the volume name is erased or not */
        if (((buffer[i * 0x20 + 0xB] & ATTR_EXT) != ATTR_EXT) && ((buffer[i * 0x20 + 0xB] & ATTR_VOLUME) != 0) &&
            (buffer[i * 0x20] != 0xE5)) {
...
int test_FAT(disk_t *disk_car, const struct fat_boot_sector *fat_header, const partition_t *partition,
...
#endif
  if (fat_header->media != 0xF0 && fat_header->media < 0xF8) { /* Legal values are 0xF0, 0xF8-0xFF */
#ifndef DISABLED_FOR_FRAMAC
  ...
  if (no_of_cluster < 4085) {
    ...
    if (fat_header->media != 0xF8) { /* the only value I have ever seen is 0xF8 */
