...
RecEnd:
  ...
  if ((param & FLAG_LIST_MASK12) != 0)
    ...
    inode &= 0xfffffff;
  if (long_slots == 0 && de->attr != ATTR_EXT) { /* short name 8.3 */
    int i;
    int j = 0;
...
static int fat_dir(disk_t *disk_car, const partition_t *partition, dir_data_t *dir_data,
  ...
  }
  {
    ...
    while (!is_EOC(cluster, partition->upart_type) && cluster >= 2 && nbr_cluster < nbr_cluster_max && stop == 0) {
      ...
      if ((unsigned)disk_car->pread(disk_car, buffer_dir + (uint64_t)cluster_size * nbr_cluster, cluster_size, start) !=
          cluster_size) {
#ifndef DISABLED_FOR_FRAMAC
      ...
      if (stop == 0) {
        if (fat_meth == FAT_FOLLOW_CLUSTER) {
          ...
          if ((next_cluster >= 2 && next_cluster <= no_of_cluster + 2) || is_EOC(next_cluster, partition->upart_type))
        ...
        if (fat_meth == FAT_NEXT_CLUSTER)
          cluster++;
        else if (fat_meth == FAT_NEXT_FREE_CLUSTER) { /* Deleted directories are composed of "free" clusters */
...
static copy_file_t fat_copy(disk_t *disk_car, const partition_t *partition, dir_data_t *dir_data,
  ...
  log_trace("fat_copy dst=%s first_cluster=%u (%llu) size=%lu\n", new_file, cluster,
            (long long unsigned)start_data + (cluster - 2) * sectors_per_cluster, (long unsigned)file_size);
#endif

  while (cluster >= 2 && cluster <= no_of_cluster + 2 && file_size > 0) {
    const uint64_t start = partition->part_offset +
                           (uint64_t)(start_data + (cluster - 2) * sectors_per_cluster) * fat_sector_size(fat_header);
    ...
    if (file_size > 0) {
      ...
      if (fat_meth == FAT_NEXT_CLUSTER)
        cluster++;
      else if (fat_meth == FAT_NEXT_FREE_CLUSTER) { /* Deleted file are composed of "free" clusters */
        while (++cluster < no_of_cluster + 2 &&
               get_next_cluster(disk_car, partition, partition->upart_type, start_fat1, cluster) != 0)
          ;
      }
