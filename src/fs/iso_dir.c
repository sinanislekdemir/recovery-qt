...
static int iso_dir(disk_t *disk, const partition_t *partition, dir_data_t *dir_data,
                   const unsigned long int first_inode, file_info_t *dir_list);
...
static void iso_ucs2be_to_utf8(char *dst, const unsigned int dst_size, const unsigned char *src,
...
static int iso_rockridge_name(const unsigned char *su, unsigned int su_len, char *dst, const unsigned int dst_size) {
  ...
  while (i + 4 <= su_len) {
    ...
    if (su[i] == 'N' && su[i + 1] == 'M' && len >= 5) {
      ...
      unsigned int k;
      if ((flags & 0x06) != 0) { /* CURRENT or PARENT flag: no usable name */
        return 0;
      }
...
static void iso_name_from_record(const unsigned char *record, const unsigned int rec_len, const unsigned int joliet,
  ...
  if (joliet) {
    ...
    if (su_off < rec_len && iso_rockridge_name(&record[su_off], rec_len - su_off, dst, dst_size) > 0) {
...
static int iso_parse_dir_block(const unsigned char *buffer, const unsigned int size, const unsigned int block_size,
                               const unsigned int joliet, file_info_t *dir_list) {
  ...
  while (offset + 33 < size) {
    ...
    if (rec_len == 0) { /* records don't cross block boundaries, skip the padding */
      ...
      break;
    {
      ...
      if ((flags & ISO_FLAG_MULTI) != 0) { /* non-final extent of a multi-extent file */
        if (multi_size == 0)
          multi_extent = extent;
        multi_size += data_len;
      } else {
        ...
        new_file->st_mode = ((flags & ISO_FLAG_DIR) != 0 ? LINUX_S_IFDIR | LINUX_S_IRUGO | LINUX_S_IXUGO
...
static int iso_dir(disk_t *disk, const partition_t *partition, dir_data_t *dir_data,
  ...
  if ((unsigned int)disk->pread(disk, buffer, dir_size, partition->part_offset + (uint64_t)extent * block_size) !=
      dir_size) {
...
dir_partition_t dir_partition_iso_init(disk_t *disk, const partition_t *partition, dir_data_t *dir_data,
  ...
  for (lba = ISO_PVD_LBA + 1; lba < ISO_PVD_LBA + 32; lba++) {
    if (disk->pread(disk, vd, ISO_BLOCK_SIZE_DFLT, partition->part_offset + (uint64_t)lba * ISO_BLOCK_SIZE_DFLT) !=
        ISO_BLOCK_SIZE_DFLT)
      break;
    if (vd[0] == 0xff)
      break; /* Volume Descriptor Set Terminator */
    if (vd[0] == 0x02 && memcmp(&vd[1], "CD001", 5) == 0 && vd[88] == 0x25 && vd[89] == 0x2f &&
