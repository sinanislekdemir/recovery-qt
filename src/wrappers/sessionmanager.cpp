...
SessionManager::SessionManager(QObject *parent) : QObject(parent), m_flags(0), m_active(false) {
...
void SessionManager::beginSession(const QString &path, scan_tree_t *tree, disk_t *disk, const partition_t *partition,
...
void SessionManager::endSession(int result) {
  ...
  session_save(m_pathBytes.constData(), m_saveCtx.tree, m_saveCtx.disk, m_saveCtx.partition, m_saveCtx.op_type,
               m_filterBytes.constData(), m_saveCtx.resume_offset, 0, m_saveCtx.resume_phase, m_saveCtx.resume_offset,
...
void SessionManager::setupResume(scan_tree_t *tree, disk_t *disk, int opType, const SessionInfo *info) {
  if (!info || !tree)
    return;
