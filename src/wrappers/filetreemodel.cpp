...
QModelIndex FileTreeModel::index(int row, int column, const QModelIndex &parent) const {
  ...
  file_node_t *parentNode = parent.isValid() ? static_cast<file_node_t *>(parent.internalPointer()) : m_tree->root;
  file_node_t *child = childNode(parentNode, row);
  if (!parent.isValid() && row < 3)
    qDebug() << "index(root," << row << "," << column << "): parentNode=" << parentNode->name
...
int FileTreeModel::rowCount(const QModelIndex &parent) const {
  if (!m_tree || !m_tree->root) {
    static int once = 0;
    if (!once++)
      qDebug() << "FileTreeModel::rowCount: m_tree is null!";
    return 0;
  }
  file_node_t *node = parent.isValid() ? static_cast<file_node_t *>(parent.internalPointer()) : m_tree->root;
  ...
  if (!parent.isValid()) {
    ...
    if (!rootOnce++)
      qDebug() << "FileTreeModel::rowCount(root)=" << cnt << "m_tree=" << (void *)m_tree;
  }
  return cnt;
}
...
QVariant FileTreeModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == MarkedRole)
    return node->marked != 0;
  if (role == DeletedRole)
    return node->deleted != 0;
  if (role == OrphanRole)
    return node->orphan != 0;
  if (role == BackupModifiedRole)
    return node->backup_modified != 0;
  if (role == FileSizeRole)
    return QVariant::fromValue(static_cast<qulonglong>(node->size));
  if (role == Qt::ToolTipRole && node->orphan) {
    return QString("Orphan/carved file at sector %1, %2 sectors").arg(node->first_sector).arg(node->num_sectors);
  }
...
QStandardItemModel *FileTreeModel::toStandardModel(QObject *parent) const {
  ...
  auto addItem = [](QStandardItemModel *m, QStandardItem *parentItem, file_node_t *node, bool filter,
                    auto &&self) -> void {
    if (!node)
      return;
...
QModelIndex FileTreeModel::findBySubstring(const QModelIndex &start, const QString &text) const {
  ...
  while (next.isValid()) {
    ...
    if (node && node->name && QString::fromUtf8(node->name).contains(text, Qt::CaseInsensitive))
      return next;
