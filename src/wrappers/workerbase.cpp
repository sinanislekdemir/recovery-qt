...
WorkerBase::WorkerBase(QObject *parent) : QObject(parent), m_running(false), m_thread(nullptr) {}
...
void WorkerBase::startThread(std::function<void()> workFn) {
  ...
  m_thread = QThread::create([this, workFn = std::move(workFn)]() { workFn(); });

  connect(m_thread, &QThread::finished, this, [this]() { disconnectAllConnections(); });
