...
static QSet<QString> previewableImageExtensions() {
  static QSet<QString> set = []() {
    QSet<QString> s;
    const char *exts[] = {"jpg", "jpeg", "png",  "gif", "bmp", "webp", "tiff", "tif", "ico", "svg", "xpm",
                          "xbm", "heic", "heif", "psd", "cr2", "nef",  "orf",  "dng", "raw", "arw", nullptr};
...
static const char *wellKnownFormats[] = {"jpg", "mov",  "png", "mp4",  "gif",    "pdf", "zip",  "doc",  "docx",
                                         "xls", "xlsx", "ppt", "pptx", "mp3",    "wav", "avi",  "bmp",  "tiff",
                                         "psd", "mxf",  "wmv", "ogg",  "flac",   "mkv", "html", "svg",  "dwg",
                                         "rar", "7z",   "tar", "gz",   "sqlite", "exe", "dll",  nullptr};
...
void SignatureRegistry::setEnabled(const QString &extension, bool enable) {
  ...
  for (fe = (file_enable_t *)m_rawArray; fe->file_hint != nullptr; fe++) {
    if (fe->file_hint->extension && strcmp(fe->file_hint->extension, ext.constData()) == 0) {
