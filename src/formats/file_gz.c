...
const file_hint_t file_hint_gz = {.extension = "gz",
...
static int header_check_bgzf(const unsigned char *buffer, const unsigned char *buffer_uncompr,
                             const unsigned int buffer_size, file_recovery_t *file_recovery_new) {
...
static int header_check_gz(const unsigned char *buffer, const unsigned int buffer_size,
...
#if defined(HAVE_ZLIB_H) && defined(HAVE_LIBZ)
  {
    static const unsigned char schematic_header[12] = {0x0a, 0x00, 0x09, 'S', 'c', 'h', 'e', 'm', 'a', 't', 'i', 'c'};
...
#endif
    if (d_stream.total_out > 0x110 && memcmp(&buffer_uncompr[0x101], tar_header_posix, sizeof(tar_header_posix)) == 0) {
#ifdef DJGPP
