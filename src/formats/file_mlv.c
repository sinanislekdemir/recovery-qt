...
const file_hint_t file_hint_mlv = {.extension = "mlv",
...
#endif
    if (le32(hdr->blockSize) < 0x10 || !is_valid_type(hdr) || fs + le32(hdr->blockSize) > file_recovery->file_size) {
      file_recovery->file_size = (fs <= file_recovery->blocksize ? 0 : fs);
      return;
    }
...
static int header_check_mlv(const unsigned char *buffer, const unsigned int buffer_size,
