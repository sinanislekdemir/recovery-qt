...
static data_check_t data_check_jpg(const unsigned char *buffer, const unsigned int buffer_size,
                                   file_recovery_t *file_recovery);

static int jpg_check_dht(const unsigned char *buffer, const unsigned int buffer_size, const unsigned i,
                         const unsigned int size);

const file_hint_t file_hint_jpg = {.extension = "jpg",
...
static void jpg_get_size(const unsigned char *buffer, const unsigned int buffer_size, unsigned int *height,
                         unsigned int *width) {
  unsigned int i = 2;
...
static uint64_t file_check_mpo_be(FILE *handle, const unsigned char *mpo, const uint64_t mpo_offset,
                                  const unsigned int size) {
...
#ifdef DEBUG_JPEG
    log_info("offset=%lu, size=%lu\n", (long unsigned)be32(MPEntry->offset), (long unsigned)be32(MPEntry->size));
#endif
    if (tmp > 0)
      tmp += mpo_offset;
    if (my_fseek(handle, tmp, SEEK_SET) < 0 || fread(buffer, sizeof(buffer), 1, handle) != 1)
      return 0;
...
static uint64_t file_check_mpo_le(FILE *handle, const unsigned char *mpo, const uint64_t mpo_offset,
                                  const unsigned int size) {
  const uint16_t *tmp16;
  /* Offset to first IFD */
  const uint32_t *tmp32 = (const uint32_t *)(&mpo[4]);
...
#ifdef DEBUG_JPEG
    log_info("mpo_offset=%lu offset=%lu, size=%lu\n", (long unsigned)mpo_offset, (long unsigned)le32(MPEntry->offset),
...
static uint64_t file_check_mpo_aux(FILE *handle, const unsigned char *mpo, const uint64_t offset,
                                   const unsigned int size) {
  /* MP header:
   * - MP Endian (4Byte)
   * - Offset to First IFD (4Byte)
...
static void file_check_mpo(file_recovery_t *fr) {
  ...
  log_info("file_check_mpo  %s calculated_file_size=%llu, error at %llu\n", fr->filename,
           (long long unsigned)fr->calculated_file_size, (long long unsigned)fr->offset_error);
#endif
  {
    /* Check the first jpg */
  ...
  } while (!(buffer[1] == 0xe2 && buffer[4] == 'M' && buffer[5] == 'P' && buffer[6] == 'F' && buffer[7] == 0));
#ifdef DEBUG_JPEG
...
static time_t jpg_get_date(const unsigned char *buffer, const unsigned int buffer_size, const unsigned int i,
                           const unsigned int size) { /* APP1 Exif information */
...
static int header_check_jpg(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (i + 4 < buffer_size && buffer[i] == 0xff && is_marker_valid(buffer[i + 1])) {
    ...
    if (buffer[i + 1] == 0xff)
      ...
      if (buffer[i + 1] == 0xe1) { /* APP1 Exif information */
  ...
  if (file_recovery->file_stat != NULL && file_recovery->file_check != NULL) {
    static const unsigned char jpg_header_app0_avi[0x0c] = {0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10,
                                                            'A',  'V',  'I',  '1',  0x00, 0x00};
    static const unsigned char jpg_header_app0_jfif11_null[0x14] = {0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 'J',
                                                                    'F',  'I',  'F',  0x00, 0x01, 0x01, 0x01,
                                                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    static const unsigned char jpg_header_app0_jfif11_com[0x17] = {0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 'J',  'F',
                                                                   'I',  'F',  0x00, 0x01, 0x01, 0x01, 0x00, 0x48,
    ...
    if (file_recovery->file_stat->file_hint == &file_hint_doc &&
        (strstr(file_recovery->filename, ".albm") != NULL || strstr(file_recovery->filename, ".prt") != NULL)) {
      if (header_ignored_adv(file_recovery, file_recovery_new) == 0)
        return 0;
    }
#endif
    if (file_recovery->file_stat->file_hint == &file_hint_jpg) {
      ...
      if (file_recovery->file_size <= 1024 && buffer[3] == 0xec) /* APP12 */
      {
#ifndef DISABLED_FOR_FRAMAC
      ...
      /* Don't recover the thumb instead of the jpg itself */
      if (file_recovery->file_size <= 16384 && width > 0 && width < 200 && height > 0 && height < 200) {
        if (header_ignored_adv(file_recovery, file_recovery_new) == 0)
          return 0;
      }
      /* Some JPG have two APP1 markers, avoid to dicard the first one */
      if (buffer[3] == 0xe1 && memcmp(&buffer[6], "http://ns.adobe.com/xap/", 24) == 0) {
        if (header_ignored_adv(file_recovery, file_recovery_new) == 0)
          return 0;
      }
    ...
    if (file_recovery->file_stat->file_hint == &file_hint_rw2 && file_recovery->file_size <= 8192) {
      if (header_ignored_adv(file_recovery, file_recovery_new) == 0)
        return 0;
    }
#endif
...
static boolean jpg_fill_input_buffer(j_decompress_ptr cinfo) {
  ...
  log_info("jpg_fill_input_buffer file_size=%llu -> %llu (offset=%llu, blocksize=%u)\n",
      ...
      src->blocksize);
#endif
  nbytes = fread(src->buffer, 1, src->blocksize - (src->offset + src->file_size) % src->blocksize, src->infile);
  if (nbytes <= 0) {
...
static void jpeg_testdisk_alloc_src(j_decompress_ptr cinfo, const unsigned int blocksize) {
  my_source_mgr *src =
      (my_source_mgr *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(my_source_mgr));
  cinfo->src = (struct jpeg_source_mgr *)src;
  src->buffer = (JOCTET *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, blocksize * sizeof(JOCTET));
}
...
static void jpeg_testdisk_src(j_decompress_ptr cinfo, FILE *infile, const uint64_t offset,
...
static uint64_t jpg_xy_to_offset(FILE *infile, const unsigned int x, const unsigned y, const uint64_t offset_rel1,
                                 const uint64_t offset_rel2, const uint64_t offset, const unsigned int blocksize) {
  ...
  while (file_size_max < offset_rel2) {
    ...
    }
    {
      ...
      if (jpeg_session.cinfo.output_scanline < jpeg_session.cinfo.output_height) {
        ...
        (void)jpeg_read_scanlines(&jpeg_session.cinfo, row_pointer, 1);
        for (i = (x + 1) * jpeg_session.output_components;
             i < jpeg_session.output_width * jpeg_session.output_components; i++) {
...
static unsigned int is_line_cut(const unsigned int output_scanline, const unsigned int output_width,
                                const unsigned int output_components, const unsigned char *frame,
  ...
  if (y + 8 < output_scanline) {
    ...
    for (x = 8 - 1; x < output_width; x += 8) {
      ...
      unsigned int j;
      for (j = 0; j < 8 && y + j < output_scanline; j++) {
        unsigned int c;
        unsigned int pos;
        for (c = 0, pos = ((y + j) * output_width + x) * output_components; c < output_components; c++, pos++)
    ...
    for (x = 8 - 1; x < output_width; x += 8) {
      ...
      for (j = 0; j < 8 && y + j < output_scanline; j++) {
        ...
        for (c = 0, pos = ((y + j) * output_width + x) * output_components; c < output_components; c++, pos++)
          result +=
              abs(2 * frame[pos] - frame[pos - output_components] -
                  (pos + output_components < end ? frame[pos + output_components] : frame[pos - output_components]));
      }
#ifdef DEBUG_JPEG2
      log_info("y=%u x=%u result=%u\n", y, x, result);
#endif
...
static unsigned int jpg_find_border(const unsigned int output_scanline, const unsigned int output_width,
                                    const unsigned int output_components, const unsigned char *frame) {
  unsigned int y;
  unsigned int val = 0;
#ifdef DEBUG_JPEG
  log_info("jpg_find_border output_scanline=%u output_width=%u output_components=%u\n", output_scanline, output_width,
           output_components);
#endif
...
static uint64_t jpg_find_error(struct jpeg_session_struct *jpeg_session, const unsigned int *offsets,
                               const uint64_t checkpoint_offset) {
  ...
  for (i = 0, pos_new = 8 * row_stride; i < row_stride; i++, pos_new++) {
  ...
  for (y = 8; y + 8 < output_scanline; y += 8) {
    unsigned int pos;
    for (i = 0, pos = y * row_stride, pos_new = (y + 8) * row_stride; i < row_stride; i++, pos++, pos_new++) {
      if (i % (8 * output_components) == 0) {
        ...
        /* 12 is a magic value */
#if 1
        else if (2 * result < result_max &&
                 result_max > 12 * row_stride) // && offset + offsets[result_x / 8] >= checkpoint_offset)
          stop = 1;
#endif
#if 1
...
static uint64_t jpg_check_thumb(FILE *infile, const uint64_t offset, const unsigned int blocksize,
                                const uint64_t checkpoint_offset, const unsigned int flags) {
  static struct my_error_mgr jerr;
  ...
  while (jpeg_session.cinfo.output_scanline < jpeg_session.cinfo.output_height) {
    ...
    src = (my_source_mgr *)jpeg_session.cinfo.src;
    src->offset_ok = src->file_size - src->pub.bytes_in_buffer;
    if (jpeg_session.cinfo.output_scanline / 8 < JPG_MAX_OFFSETS &&
        offsets[jpeg_session.cinfo.output_scanline / 8] == 0)
...
static void jpg_check_picture(file_recovery_t *file_recovery) {
  ...
  while (jpeg_session.cinfo.output_scanline < jpeg_session.cinfo.output_height) {
    ...
    if (jpeg_session.flags == 0)
      ...
      row_pointer[0] =
          (unsigned char *)jpeg_session.frame + jpeg_session.cinfo.output_scanline * jpeg_session.row_stride;
    (void)jpeg_read_scanlines(&jpeg_session.cinfo, row_pointer, 1);
  }
  {
    my_source_mgr *src;
...
static int jpg_check_dht(const unsigned char *buffer, const unsigned int buffer_size, const unsigned int i,
                         const unsigned int size) {
  unsigned int j = i + 4;
...
static int jpg_check_sof0(const unsigned char *buffer, const unsigned int buffer_size, const unsigned int i) {
    ...
    return 0;
  {
    ...
    if (h->precision != 8 || be16(h->width) == 0 || h->nbr == 0)
      return 1;
    if (length < 8 + (unsigned int)h->nbr * 3)
      return 1;
  }
...
static void jpg_search_marker(file_recovery_t *file_recovery) {
  ...
  while ((nbytes = fread(&sbuffer, 1, sizeof(sbuffer), infile)) > 0) {
    ...
    while (i + 1 < nbytes) {
      ...
      if (buffer[i] == 0xff && (buffer[i + 1] == 0xd8 ||                            /* SOI */
                                buffer[i + 1] == 0xdb ||                            /* DQT */
...
static void jpg_save_thumbnail(const file_recovery_t *file_recovery, const char *buffer, const uint64_t nbytes,
                               const uint64_t thumb_offset, const unsigned int thumb_size) {
  char thumbname[2048];
  char *sep;
...
static int jpg_check_app1(file_recovery_t *file_recovery, const unsigned int extract_thumb, const unsigned char *buffer,
                          const unsigned int i, const unsigned int offset, const unsigned int size,
                          const uint64_t nbytes, uint64_t *thumb_offset_ptr) { /* APP1 Exif information */
  const unsigned int tiff_offset = i + 2 + 8;
...
static uint64_t jpg_check_structure(file_recovery_t *file_recovery, const unsigned int extract_thumb) {
  ...
  for (offset = file_recovery->blocksize; offset + 30 < nbytes && file_recovery->offset_error == 0;
       offset += file_recovery->blocksize) {
  ...
  while (offset + 4 < nbytes && buffer[offset] == 0xff && is_marker_valid(buffer[offset + 1]) &&
         (file_recovery->offset_error == 0 || offset < file_recovery->offset_error)) {
...
static void file_check_jpg(file_recovery_t *file_recovery) {
  ...
  log_info("file_check_jpg  %s calculated_file_size=%llu, error at %llu\n", file_recovery->filename,
           (long long unsigned)file_recovery->calculated_file_size, (long long unsigned)file_recovery->offset_error);
#endif
...
#endif
#if defined(HAVE_LIBJPEG) && defined(HAVE_JPEGLIB_H)
  if (thumb_offset != 0 && (file_recovery->checkpoint_status == 0 || thumb_error != 0) &&
      (file_recovery->offset_error == 0 || thumb_offset < file_recovery->offset_error)) {
#ifdef DEBUG_JPEG
    log_info("jpg_check_thumb\n");
#endif
    thumb_error = jpg_check_thumb(file_recovery->handle, thumb_offset, file_recovery->blocksize,
                                  file_recovery->checkpoint_offset, file_recovery->flags);
    if (thumb_error != 0) {
#ifdef DEBUG_JPEG
      log_info("%s thumb corrupted at %llu, previous error at %llu\n", file_recovery->filename,
               (long long unsigned)thumb_error, (long long unsigned)file_recovery->offset_error);
#endif
...
static data_check_t data_check_continue(const unsigned char *buffer, const unsigned int buffer_size,
                                        file_recovery_t *file_recovery) {
...
static data_check_t data_check_jpg2(const unsigned char *buffer, const unsigned int buffer_size,
                                    file_recovery_t *file_recovery) {
...
static data_check_t data_check_jpg(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
    ...
    if (buffer[i] == 0xFF && buffer[i + 1] == 0xFF)
      ...
      {
        if (i + 8 < buffer_size && buffer[i + 4] == 'M' && buffer[i + 5] == 'P' && buffer[i + 6] == 'F' &&
            buffer[i + 7] == 0) {
