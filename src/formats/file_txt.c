...
const file_hint_t file_hint_snz = {.extension = "snz",
...
const file_hint_t file_hint_txt = {.extension = "txt",
                                   .description =
                                       "Other text files: txt,html,asp,bat,C,jsp,perl,php,py/emlx... scripts",
                                   .max_filesize = PHOTOREC_MAX_FILE_SIZE,
                                   .recover = 1,
                                   .enable_by_default = 1,
...
static int filtre(unsigned int car) {
  ...
  if (car == '\b' || car == '\t' || car == '\r' || car == '\n' || (car >= ' ' && car <= '~') ||
      (car >= 0x82 && car <= 0x8d) || (car >= 0x93 && car <= 0x98))
    return 1;
  return 0;
}
...
static int UTF2Lat(unsigned char *buffer_lower, const unsigned char *buffer, const int buf_len) {
  ...
  for (offset_dst = 0, p = buffer, q = buffer_lower;
       p + 2 - buffer < buf_len && offset_dst < (unsigned int)(buf_len - 1) && *p != '\0';) {
    if ((*p & 0xf0) == 0xe0 && (*(p + 1) & 0xc0) == 0x80 && (*(p + 2) & 0xc0) == 0x80) { /* UTF8 l=3 */
#ifdef DEBUG_TXT
      ...
      *q = '\0';
      ...
      p += 3;
    } else if ((*p & 0xe0) == 0xc0 && (*(p + 1) & 0xc0) == 0x80) { /* UTF8 l=2 */
      *q = '\0';
      switch (*p) {
      case 0xC2: {
...
static data_check_t data_check_html(const unsigned char *buffer, const unsigned int buffer_size,
                                    file_recovery_t *file_recovery) {
  const char sign_html_end[] = "</html>";
  if (buffer_size / 2 > (sizeof(sign_html_end) - 1)) {
    unsigned int j;

    for (j = buffer_size / 2 - (sizeof(sign_html_end) - 1); j + sizeof(sign_html_end) - 1 < buffer_size; j++) {
...
static data_check_t data_check_ttd(const unsigned char *buffer, const unsigned int buffer_size,
                                   file_recovery_t *file_recovery) {
  unsigned int i;
...
static data_check_t data_check_txt(const unsigned char *buffer, const unsigned int buffer_size,
                                   file_recovery_t *file_recovery) {
  const unsigned int i = UTFsize(&buffer[buffer_size / 2], buffer_size / 2);
  if (i < buffer_size / 2) {
    if (i >= 10)
...
static data_check_t data_check_xml_utf8(const unsigned char *buffer, const unsigned int buffer_size,
                                        file_recovery_t *file_recovery) {
  unsigned int i;
  if (buffer_size <= 8)
    return DC_CONTINUE;
...
static void file_rename_fods(file_recovery_t *file_recovery) {
  ...
  for (tmp = strchr(buffer, '<'); tmp != NULL && strncasecmp(tmp, meta_title, 23) != 0; tmp = strchr(tmp, '<')) {
    /* TODO assert tmp[0]=='<'; */
...
static int header_check_dc(const unsigned char *buffer, const unsigned int buffer_size,
...
static int header_check_ers(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
...
static int header_check_fasttxt(const unsigned char *buffer, const unsigned int buffer_size,
                                const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                file_recovery_t *file_recovery_new) {
...
static int header_check_html(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (buffer_size < 15)
    return 0;
  if (file_recovery->file_stat != NULL && file_recovery->file_stat->file_hint == &file_hint_fasttxt &&
...
static int header_check_ics(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  const char *date_asc;
  char *buffer2;
...
static int header_check_le16_txt(const unsigned char *buffer, const unsigned int buffer_size,
                                 const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                 file_recovery_t *file_recovery_new) {
  unsigned int i;
...
static int header_check_mbox(const unsigned char *buffer, const unsigned int buffer_size,
                             const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                             file_recovery_t *file_recovery_new) {
  unsigned int i;
  if (buffer_size < 200)
  ...
  for (i = 0; i < 64; i++)
    if (buffer[i] == 0)
      return 0;
  if (memcmp(buffer, "From ", 5) == 0 && memcmp(buffer, "From MAILER-DAEMON ", 19) != 0) {
...
static int header_check_mol2(const unsigned char *buffer, const unsigned int buffer_size,
                             const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                             file_recovery_t *file_recovery_new) {
  reset_file_recovery(file_recovery_new);
  file_recovery_new->data_check = &data_check_txt;
...
static int header_check_perlm(const unsigned char *buffer, const unsigned int buffer_size,
                              const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                              file_recovery_t *file_recovery_new) {
  unsigned int i;
...
static int header_check_rtf(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  unsigned int i;
  if (buffer_size < 16)
...
static int header_check_smil(const unsigned char *buffer, const unsigned int buffer_size,
                             const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                             file_recovery_t *file_recovery_new) {
...
static int header_check_snz(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
...
static int header_check_stl(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  const unsigned int buffer_size_test = (buffer_size < 512 ? buffer_size : 512);
...
static int header_check_svg(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
...
static int header_check_thunderbird(const unsigned char *buffer, const unsigned int buffer_size,
                                    const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                    file_recovery_t *file_recovery_new) {
  unsigned int i;
  if (buffer_size < 64)
...
static int header_check_ttd(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
...
static int header_check_txt(const unsigned char *buffer, const unsigned int buffer_size,
    ...
    return 0;
  {
    ...
    if (buffer[i] == 0x0a &&
        (memcmp(buffer + i + 1, "Return-Path: ", 13) == 0 || memcmp(buffer + i + 1, "Received: from", 14) == 0) &&
        !(file_recovery->file_stat != NULL && file_recovery->file_stat->file_hint == &file_hint_fasttxt &&
          file_recovery->extension == extension_mbox)) {
      reset_file_recovery(file_recovery_new);
  ...
  }
  {
    ...
    /* Detect LaTeX, C, PHP, JSP, ASP, HTML, C header */
      ...
      ext = extension_html;
    else if (strstr(buffer_lower, "private static") != NULL || strstr(buffer_lower, "public interface") != NULL) {
      ext = extension_java;
      ...
      if (*str == ';')
        ...
        ext = extension_py;
    } else if (strstr(buffer_lower, "class ") != NULL && (l >= 100 || file_recovery->file_stat == NULL)) {
      ext = extension_java;
    }
    /* Fortran */
...
static int header_check_vbm(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  reset_file_recovery(file_recovery_new);
  file_recovery_new->data_check = &data_check_txt;
...
static int header_check_xml(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
...
static int header_check_xml_utf8(const unsigned char *buffer, const unsigned int buffer_size,
                                 const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                 file_recovery_t *file_recovery_new) {
  /* buffer may not be null-terminated */
...
static int header_check_xml_utf16(const unsigned char *buffer, const unsigned int buffer_size,
                                  const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                  file_recovery_t *file_recovery_new) {
...
static int header_check_xmp(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  if (buffer[35] == '\0')
    return 0;
...
#if !defined(MAIN_txt) && !defined(SINGLE_FORMAT)
      &&
      (file_recovery->file_stat->file_hint == &file_hint_jpg || file_recovery->file_stat->file_hint == &file_hint_pdf ||
...
static void register_header_check_fasttxt(file_stat_t *file_stat) {
  static const unsigned char header_xml_utf8[17] = {0xef, 0xbb, 0xbf, '<', '?', 'x', 'm', 'l', ' ',
                                                    'v',  'e',  'r',  's', 'i', 'o', 'n', '='};
  static const unsigned char header_xml_utf16[30] = {0xff, 0xfe, '<', 0, '?', 0, 'x', 0, 'm', 0, 'l', 0, ' ', 0, 'v', 0,
                                                     'e',  0,    'r', 0, 's', 0, 'i', 0, 'o', 0, 'n', 0, '=', 0};
  const txt_header_t *header = &fasttxt_headers[0];
