...
const file_hint_t file_hint_edb = {.extension = "edb",
...
static int header_check_edb(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  /* A backup of the header is stored in the second block */
  if (file_recovery->file_stat != NULL && file_recovery->file_stat->file_hint == &file_hint_edb &&
...
static void register_header_check_edb(file_stat_t *file_stat) {
  static const unsigned char edb_magic[8] = {0xef, 0xcd, 0xab, 0x89, 0x20, 0x06, 0x00, 0x00};
