...
const file_hint_t file_hint_mid = {.extension = "mid",
...
static void file_check_midi(file_recovery_t *file_recovery) {
  ...
  if (my_fseek(file_recovery->handle, 0, SEEK_SET) < 0 || fread(&hdr, sizeof(hdr), 1, file_recovery->handle) != 1)
    return;
...
#endif
    if (my_fseek(file_recovery->handle, fs, SEEK_SET) < 0 || fread(&track, 8, 1, file_recovery->handle) != 1 ||
...
static data_check_t data_check_midi(const unsigned char *buffer, const unsigned int buffer_size,
...
static int header_check_mid(const unsigned char *buffer, const unsigned int buffer_size,
