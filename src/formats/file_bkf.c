...
const file_hint_t file_hint_bkf = {.extension = "bkf",
...
static int header_check_bkf(const unsigned char *buffer, const unsigned int buffer_size,
   ...
   */
  if (le64(hdr->fla) != 0 || le32(hdr->cbId) != 0 || hdr->strType > 2 || le16(hdr->off) < sizeof(struct mtf_db_hdr))
    return 0;
#ifdef DEBUG_BKF
  log_info("off=%u\n", le16(hdr->off));
#endif
