...
const file_hint_t file_hint_oci = {.extension = "oci",
...
static data_check_t data_check_oci(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
    ...
    log_trace("file_oci.c: %s atom %c%c%c%c (0x%02x%02x%02x%02x) size %llu, calculated_file_size %llu\n",
              file_recovery->filename, buffer[i], buffer[i + 1], buffer[i + 2], buffer[i + 3], buffer[i], buffer[i + 1],
              buffer[i + 2], buffer[i + 3], (long long unsigned)atom_size,
              (long long unsigned)file_recovery->calculated_file_size);
#endif
    if (buffer[i + 0] == 'O' && (buffer[i + 1] >= 'A' && buffer[i + 1] <= 'Z') &&
        (buffer[i + 2] >= 'A' && buffer[i + 2] <= 'Z') && (buffer[i + 3] >= 'A' && buffer[i + 3] <= 'Z')) {
...
static int header_check_oci(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (8 + size + 8 <= buffer_size) {
    ...
    if (!(hdr2->type[0] == 'O' && (hdr2->type[1] >= 'A' && hdr2->type[1] <= 'Z') &&
          (hdr2->type[2] >= 'A' && hdr2->type[2] <= 'Z') && (hdr2->type[3] >= 'A' && hdr2->type[3] <= 'Z')))
      return 0;
  }
...
static void register_header_check_oci(file_stat_t *file_stat) {
  static const unsigned char oci_header[8] = {'O', 'P', 'I', 'M', '0', 0x00, 0x00, 0x00};
