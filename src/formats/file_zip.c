...
const file_hint_t file_hint_zip = {.extension = "zip",
...
static int zip_parse_file_entry_fn(file_recovery_t *fr, const char **ext, const unsigned int file_nbr,
                                   const zip_file_entry_t *file, const uint64_t len) {
...
#ifdef DEBUG_ZIP
  log_info("%u Comp=%u %u CRC32=0x%08X extra_length=%u ", le32(file->compressed_size), le16(file->compression),
           le32(file->uncompressed_size), le32(file->crc32), le16(file->extra_length));
#endif
...
#endif
    if (my_fseek(fr->handle, fr->file_size, SEEK_SET) == -1 || my_fseek(fr->handle, len, SEEK_CUR) == -1) {
#ifdef DEBUG_ZIP
      log_trace("zip: Unexpected EOF in file_entry header: %lu bytes expected\n", len);
#endif
      return -1;
...
static int zip_parse_data_desc(file_recovery_t *fr) {
  ...
  log_info("compressed_size=%u/%lu uncompressed_size=%u CRC32=0x%08X\n", le32(desc->compressed_size),
           expected_compressed_size, le32(desc->uncompressed_size), le32(desc->crc32));
#endif
...
static void file_rename_zip(file_recovery_t *file_recovery) {
  ...
  while (1) {
    ...
    if (header == ZIP_END_CENTRAL_DIR) {
      ...
      for (len = 0; len < 32 && first_filename[len] != '\0' && first_filename[len] != '.' &&
                    first_filename[len] != '/' && first_filename[len] != '\\';
           len++)
        ;
...
static int header_check_zip(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (file_recovery->file_check == &file_check_zip && file_recovery->file_stat != NULL &&
...
static int header_check_winzip(const unsigned char *buffer, const unsigned int buffer_size,
                               const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                               file_recovery_t *file_recovery_new) {
  reset_file_recovery(file_recovery_new);
  file_recovery_new->file_check = &file_check_zip;
...
static void register_header_check_zip(file_stat_t *file_stat) {
  static const unsigned char zip_header2[8] = {'P', 'K', '0',  '0',
                                               'P', 'K', 0x03, 0x04}; /* WinZIPv8-compressed files. */
