...
const file_hint_t file_hint_pdb = {.extension = "pdb",
...
static data_check_t data_check_pdb81(const unsigned char *buffer, const unsigned int buffer_size,
...
static data_check_t data_check_pdb82(const unsigned char *buffer, const unsigned int buffer_size,
                                     file_recovery_t *file_recovery) {
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
...
static int header_check_pdb(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (buffer[0x34] != '-')
    return 0;
  if (buffer[0x35] < 'A' || buffer[0x35] > 'Z' || buffer[0x36] < 'A' || buffer[0x36] > 'Z' || buffer[0x37] < 'A' ||
      buffer[0x37] > 'Z')
    return 0;
  if (buffer[0x38] != '-')
    return 0;
  if (buffer[0x39] < '0' || buffer[0x39] > '9' || buffer[0x3a] < '0' || buffer[0x3a] > '9')
    return 0;
  /* Check space */
  if (buffer[59] != ' ' || buffer[60] != ' ' || buffer[61] != ' ' || buffer[66] != ' ' || buffer[67] != ' ' ||
      buffer[68] != ' ' || buffer[69] != ' ')
    return 0;
