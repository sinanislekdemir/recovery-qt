...
const file_hint_t file_hint_exe = {.extension = "exe",
...
static char vs_version_info[32] = {'V', 0x0, 'S', 0x0, '_', 0x0, 'V', 0x0, 'E', 0x0, 'R', 0x0, 'S', 0x0, 'I', 0x0,
                                   'O', 0x0, 'N', 0x0, '_', 0x0, 'I', 0x0, 'N', 0x0, 'F', 0x0, 'O', 0x0, 0x0, 0x0};

static char StringFileInfo[30] = {'S', 0x0, 't', 0x0, 'r', 0x0, 'i', 0x0, 'n', 0x0, 'g', 0x0, 'F', 0x0, 'i',
                                  0x0, 'l', 0x0, 'e', 0x0, 'I', 0x0, 'n', 0x0, 'f', 0x0, 'o', 0x0, 0x0, 0x0};

static char OriginalFilename[34] = {'O', 0x0, 'r', 0x0, 'i', 0x0, 'g', 0x0, 'i', 0x0, 'n', 0x0,
                                    'a', 0x0, 'l', 0x0, 'F', 0x0, 'i', 0x0, 'l', 0x0, 'e', 0x0,
                                    'n', 0x0, 'a', 0x0, 'm', 0x0, 'e', 0x0, 0x0, 0x0};

static char InternalName[24] = {'I', 0x0, 'n', 0x0, 't', 0x0, 'e', 0x0, 'r', 0x0, 'n', 0x0,
                                'a', 0x0, 'l', 0x0, 'N', 0x0, 'a', 0x0, 'm', 0x0, 'e', 0x0};

static int parse_String(file_recovery_t *file_recovery, const char *buffer, const unsigned int end, const char *needle,
                        const unsigned int needle_len, const int force_ext) {
...
static int parse_StringArray(file_recovery_t *file_recovery, const char *buffer, const unsigned int end,
                             const char *needle, const unsigned int needle_len, const int force_ext) {
  unsigned int pos = 0;
#ifdef DEBUG_EXE
...
static int parse_StringTable(file_recovery_t *file_recovery, const char *buffer, const unsigned int end,
                             const char *needle, const unsigned int needle_len, const int force_ext) {
  const struct PE_index *PE_index;
  unsigned int pos;
  unsigned int len;
...
static int parse_StringFileInfo(file_recovery_t *file_recovery, const char *buffer, const unsigned int end,
                                const char *needle, const unsigned int needle_len, const int force_ext) {
...
static int parse_VS_VERSIONINFO(file_recovery_t *file_recovery, const char *buffer, const unsigned int end,
                                const char *needle, const unsigned int needle_len, const int force_ext) {
...
static void PEVersion(FILE *file, const unsigned int offset, const unsigned int length,
...
#endif
  if (parse_VS_VERSIONINFO(file_recovery, (const char *)&buffer, length, OriginalFilename, sizeof(OriginalFilename),
                           0) == 0) {
    return;
  }
...
static int pe_resource_language_aux(FILE *file, const unsigned int base, const struct pe_image_section_hdr *pe_sections,
                                    const unsigned int nbr_sections, file_recovery_t *file_recovery,
                                    const struct rsrc_entries_s *rsrc_entry) {
  struct rsrc_offlen buffer;
  uint32_t off;
  unsigned int len;
  unsigned int j;
#ifdef DEBUG_EXE
  log_info("resource lang=%u, %x, offset %u\n", le32(rsrc_entry->Type), le32(rsrc_entry->Pos),
  ...
  for (j = 0; j < nbr_sections; j++) {
    ...
    if (virt_addr_end <= 0xffffffff && virt_addr_start <= off && off < virt_addr_end &&
        (uint64_t)off - virt_addr_start + base <= 0xffffffff) {
...
static void pe_resource_language(FILE *file, const unsigned int base, const unsigned int dir_start,
                                 const struct pe_image_section_hdr *pe_sections, const unsigned int nbr_sections,
                                 file_recovery_t *file_recovery) {
...
static void pe_resource_id(FILE *file, const unsigned int base, const unsigned int dir_start,
                           const struct pe_image_section_hdr *pe_sections, const unsigned int nbr_sections,
                           file_recovery_t *file_recovery) {
  struct rsrc_entries_s *rsrc_entries;
  unsigned int count;
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((le32(rsrc_entry->Pos) & 0x80000000) != 0) {
      pe_resource_language(file, base, le32(rsrc_entry->Pos) & 0x7fffffff, pe_sections, nbr_sections, file_recovery);
    }
  }
  free(rsrc_entries);
}

static void pe_resource_type(FILE *file, const unsigned int base, const unsigned int dir_start,
                             const struct pe_image_section_hdr *pe_sections, const unsigned int nbr_sections,
                             file_recovery_t *file_recovery) {
  struct rsrc_entries_s *rsrc_entries;
  unsigned int count;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (rsrcType == 16) {
      if ((le32(rsrc_entry->Pos) & 0x80000000) != 0) {
        pe_resource_id(file, base, le32(rsrc_entry->Pos) & 0x7fffffff, pe_sections, nbr_sections, file_recovery);
      }
    }
  }
  free(rsrc_entries);
  ...
  }
  {
    ...
    for (i = 0; i < nbr_sections; i++) {
      ...
      if (le32(pe_section->VirtualSize) > 0) {
        log_info("%s 0x%lx-0x%lx\n", pe_section->Name, (unsigned long)le32(pe_section->VirtualAddress),
    ...
    for (i = 0; i < nbr_sections; i++) {
      ...
      if (le32(pe_section->SizeOfRawData) > 0) {
        if (memcmp((const char *)pe_section->Name, ".rsrc", 6) == 0) {
          pe_resource_type(file, le32(pe_section->PointerToRawData), le32(pe_section->SizeOfRawData), pe_sections,
                           nbr_sections, file_recovery);
          fclose(file);
          return;
        }
      }
...
static int header_check_exe(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  const struct dos_image_file_hdr *dos_hdr = (const struct dos_image_file_hdr *)buffer;
  if (memcmp(buffer, exe_header, sizeof(exe_header)) != 0)
    return 0;
  if (le32(dos_hdr->e_lfanew) > 0 && le32(dos_hdr->e_lfanew) <= buffer_size - sizeof(struct pe_image_file_hdr)) {
    ...
    if ((le32(pe_hdr->Magic) & 0xffff) == IMAGE_NT_SIGNATURE) {
...
#endif
      {
        ...
        const struct pe_image_section_hdr *pe_image_section =
            (const struct pe_image_section_hdr *)((const unsigned char *)pe_hdr + sizeof(struct pe_image_file_hdr) +
                                                  le16(pe_hdr->SizeOfOptionalHeader));

        for (i = 0; i < le16(pe_hdr->NumberOfSections) &&
                    (const unsigned char *)(pe_image_section + 1) <= buffer + buffer_size;
             i++, pe_image_section++) {
          if (le32(pe_image_section->SizeOfRawData) > 0) {
            const uint64_t tmp =
                (uint64_t)le32(pe_image_section->PointerToRawData) + le32(pe_image_section->SizeOfRawData);
#ifdef DEBUG_EXE
            log_debug("%s 0x%lx-0x%lx\n", pe_image_section->Name,
                      (unsigned long)le32(pe_image_section->PointerToRawData), (unsigned long)(tmp - 1));
#endif
          ...
          if (le16(pe_image_section->NumberOfRelocations) > 0) {
            const uint64_t tmp = (uint64_t)le32(pe_image_section->PointerToRelocations) +
                                 1 * le16(pe_image_section->NumberOfRelocations);
        ...
        if (le32(pe_hdr->NumberOfSymbols) > 0) {
          const uint64_t tmp = (uint64_t)le32(pe_hdr->PointerToSymbolTable) +
                               IMAGE_SIZEOF_SYMBOL * (uint64_t)le32(pe_hdr->NumberOfSymbols);
  ...
  if (le16(dos_hdr->bytes_in_last_block) <= 512 && le16(dos_hdr->blocks_in_file) > 0 &&
    ...
    if (coff_offset < buffer_size - 1 && buffer[coff_offset] == 0x4c &&
        buffer[coff_offset + 1] == 0x01) { /*  COFF_I386MAGIC */
      reset_file_recovery(file_recovery_new);
      file_recovery_new->extension = file_hint_exe.extension;
