...
const file_hint_t file_hint_asf = {.extension = "asf",
...
static int header_check_asf(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (le64(hdr->object_size) < 30 || le64(hdr->object_size) >= PHOTOREC_MAX_FILE_SIZE || nbr_header_obj < 4)
    return 0;

  for (i = 0; i < nbr_header_obj && offset_prop + 0x28 < buffer_size; i++) {
    ...
    static const unsigned char asf_file_prop_id[16] = {0xa1, 0xdc, 0xab, 0x8c, 0x47, 0xa9, 0xcf, 0x11,
                                                       0x8e, 0xe4, 0x00, 0xc0, 0x0c, 0x20, 0x53, 0x65};
    static const unsigned char asf_stream_prop_s[16] = {0x91, 0x07, 0xdc, 0xb7, 0xb7, 0xa9, 0xcf, 0x11,
    ...
    if (memcmp(prop->object_id, asf_file_prop_id, sizeof(asf_file_prop_id)) == 0) {
      ...
      const unsigned char wma[16] = {0x40, 0x9e, 0x69, 0xf8, 0x4d, 0x5b, 0xcf, 0x11,
                                     0xa8, 0xfd, 0x00, 0x80, 0x5f, 0x5c, 0x44, 0x2b};
      const unsigned char wmv[16] = {0xc0, 0xef, 0x19, 0xbc, 0x4d, 0x5b, 0xcf, 0x11,
                                     0xa8, 0xfd, 0x00, 0x80, 0x5f, 0x5c, 0x44, 0x2b};
      if (object_size < 0x28)
        return 0;
...
static void register_header_check_asf(file_stat_t *file_stat) {
  static const unsigned char asf_header[16] = {0x30, 0x26, 0xb2, 0x75, 0x8e, 0x66, 0xcf, 0x11,
