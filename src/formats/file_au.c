...
const file_hint_t file_hint_au = {.extension = "au",
...
static int header_check_au(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (offset >= sizeof(struct header_au_s) && be32(au->encoding) > 0 && be32(au->encoding) <= 27 &&
