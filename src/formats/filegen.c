...
static file_check_t file_check_plist = {.list = TD_LIST_HEAD_INIT(file_check_plist.list)};

file_check_list_t file_check_list = {.list = TD_LIST_HEAD_INIT(file_check_list.list)};
...
static void index_header_check_aux(file_check_t *file_check_new) {
  ...
  if (file_check_new->length > 0) {
    ...
    td_list_for_each(tmp, &file_check_list.list) {
      ...
      if (pos->offset >= file_check_new->offset && pos->offset < file_check_new->offset + file_check_new->length) {
#ifdef __FRAMAC__
        td_list_add_sorted_fcc(
            &file_check_new->list,
            &pos->file_checks[((const unsigned char *)file_check_new->value)[pos->offset - file_check_new->offset]]
                 .list);
#else
        td_list_add_sorted(
            &file_check_new->list,
...
void file_search_footer(file_recovery_t *file_recovery, const void *footer, const unsigned int footer_length,
  ...
  if (footer_length == 0 || file_recovery->file_size <= extra_length)
    return;
  file_recovery->file_size =
      file_rsearch(file_recovery->handle, file_recovery->file_size - extra_length, footer, footer_length);
...
static int _file_rename(char *filename, const void *buffer, const int buffer_size, const int offset,
                        const char *new_ext, const int append_original_ext) {
  char *new_filename;
...
static int _file_rename_unicode(file_recovery_t *file_recovery, const void *buffer, const int buffer_size,
                                const int offset, const char *new_ext, const int append_original_ext) {
  char *new_filename;
...
void get_prev_location_smart(const alloc_data_t *list_search_space, alloc_data_t **current_search_space,
  ...
  log_info("get_prev_location_smart: reset offset_skipped_header=%llu, offset=%llu\n",
           (long long unsigned)(offset_skipped_header / 512), (long long unsigned)(*offset / 512));
#endif
...
#ifdef DEBUG_PREV_LOCATION
      log_info("get_prev_location_smart: file_space->start < offset_skipped_header, prev_location=%llu (in 512-bytes "
               "sectors), offset=%llu => %llu\n",
               (long long unsigned)(prev_location / 512), (long long unsigned)(*offset / 512),
...
time_t get_time_from_YYYY_MM_DD_HH_MM_SS(const unsigned char *date_asc) {
  ...
  tm_time.tm_year = (date_asc[0] - '0') * 1000 + (date_asc[1] - '0') * 100 + (date_asc[2] - '0') * 10 +
                    (date_asc[3] - '0') - 1900; /* year */
  tm_time.tm_isdst = -1;                        /* unknown daylight saving time */
  return mktime(&tm_time);
}
