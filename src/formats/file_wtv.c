...
const file_hint_t file_hint_wtv = {.extension = "wtv",
...
static int header_check_wtv(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (block == 0)
    return 0;
  if (file_recovery->file_stat != NULL && file_recovery->file_stat->file_hint == &file_hint_wtv &&
...
static void register_header_check_wtv(file_stat_t *file_stat) {
  static const unsigned char wtv_header[16] = {0xb7, 0xd8, 0x00, ' ',  '7',  'I',  0xda, 0x11,
