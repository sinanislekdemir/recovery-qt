...
const file_hint_t file_hint_mat = {.extension = "mat",
                                   .description = "Matlab",
...
static int header_check_mat(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  const unsigned char mat_le[4] = {0x00, 0x01, 'I', 'M'};
  const unsigned char mat_be[4] = {0x01, 0x00, 'M', 'I'};
  if (memcmp(&buffer[0x7c], mat_le, sizeof(mat_le)) == 0 || memcmp(&buffer[0x7c], mat_be, sizeof(mat_be)) == 0) {
