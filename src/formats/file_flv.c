...
const file_hint_t file_hint_flv = {.extension = "flv",
...
static data_check_t data_check_flv(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
    ...
    if ((be32(tag->prev_tag_size) == 0 && file_recovery->calculated_file_size < buffer_size / 2) ||
      ...
      if ((tag->info & 0xc0) != 0 || file_recovery->data_check_tmp == 0 || tag->streamID[0] != 0 ||
          tag->streamID[1] != 0 || tag->streamID[2] != 0) {
...
static int header_check_flv(const unsigned char *buffer, const unsigned int buffer_size,
