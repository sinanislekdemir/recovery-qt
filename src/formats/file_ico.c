...
const file_hint_t file_hint_ico = {.extension = "ico",
...
static int header_check_ico(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  for (i = 0, ico_dir = (const struct ico_directory *)(ico + 1);
       (const unsigned char *)(ico_dir + 1) <= buffer + buffer_size && i < le16(ico->count); i++, ico_dir++) {
#ifdef DEBUG_ICO
    log_info("ICO%u: reserved=%u color_planes=%u width=%u heigth=%u bps=%u offset=%u size=%u\n", i, ico_dir->reserved,
             le16(ico_dir->color_planes), ico_dir->width, ico_dir->heigth, le16(ico_dir->bits_per_pixel),
