...
const file_hint_t file_hint_fcs = {.extension = "fcs",
...
static int header_check_fcs(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (!(text_start <= text_end && data_start <= data_end && analysis_start <= analysis_end))
    return 0;
  if ((data_end == 0 || analysis_end == 0) && text_start < buffer_size) { /* Explore TEXT segment */
    unsigned int i;
    ...
    for (i = 0; i < smallest; i++) {
      if (buffer[i] == delimiter) {
        if (i + 1 + 8 + 1 < smallest && memcmp(buffer + i + 1, "$ENDDATA", 8) == 0 && buffer[i + 1 + 8] == delimiter)
          data_end = ascii2int2(&buffer[i + 1 + 8 + 1], smallest - (i + 1 + 8 + 1), delimiter);
        else if (i + 1 + 9 + 1 < smallest && memcmp(buffer + i + 1, "$ENDSTEXT", 9) == 0 &&
                 buffer[i + 1 + 9] == delimiter)
          stext_end = ascii2int2(&buffer[i + 1 + 9 + 1], smallest - (i + 1 + 9 + 1), delimiter);
        else if (i + 1 + 12 + 1 < smallest && memcmp(buffer + i + 1, "$ENDANALYSIS", 12) == 0 &&
                 buffer[i + 1 + 12] == delimiter)
...
#endif
  if (data_end >= 0x8000000000000000 - 9 || analysis_end >= 0x8000000000000000 - 9 ||
