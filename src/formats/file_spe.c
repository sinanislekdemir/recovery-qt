...
const file_hint_t file_hint_spe = {.extension = "spe",
...
static int header_check_spe(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  log_debug("spe xdim=%u ydim=%u NumFrames=%u datatype=%u size=%llu\n", le16(spe->xdim), le16(spe->ydim),
            (unsigned int)le32(spe->NumFrames), le16(spe->datatype),
