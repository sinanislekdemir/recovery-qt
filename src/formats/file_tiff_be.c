...
static unsigned int get_nbr_fields_be(const unsigned char *buffer, const unsigned int tiff_size,
...
static unsigned int find_tag_from_tiff_header_be_aux(const unsigned char *buffer, const unsigned int tiff_size,
...
unsigned int find_tag_from_tiff_header_be(const unsigned char *buffer, const unsigned int tiff_size,
                                          const unsigned int tag, const unsigned char **potential_error) {
...
static uint64_t parse_strip_be(FILE *handle, const TIFFDirEntry *entry_strip_offsets,
                               const TIFFDirEntry *entry_strip_bytecounts) {
  const unsigned int nbr =
      (be32(entry_strip_offsets->tdir_count) < 2048 ? be32(entry_strip_offsets->tdir_count) : 2048);
  ...
  if (nbr == 0 || be16(entry_strip_offsets->tdir_type) != 4 || be16(entry_strip_bytecounts->tdir_type) != 4)
    return TIFF_ERROR;
...
#ifdef DEBUG_TIFF
    log_info("%u tag=%u(0x%x) %s type=%u count=%lu offset=%lu(0x%lx)\n", i, be16(entry->tdir_tag),
             be16(entry->tdir_tag), tag_name(be16(entry->tdir_tag)), be16(entry->tdir_type),
             (long unsigned)be32(entry->tdir_count), (long unsigned)be32(entry->tdir_offset),
...
static uint64_t file_check_tiff_be_aux(file_recovery_t *fr, const uint32_t tiff_diroff, const unsigned int depth,
  ...
  for (i = 0; i < n && i < (unsigned int)(data_read - 2) / 12; i++) {
    ...
    log_info("%u tag=%u(0x%x) %s type=%u count=%lu offset=%lu(0x%lx) val=%lu\n", i, tdir_tag, tdir_tag,
             tag_name(tdir_tag), be16(entry->tdir_type), (long unsigned)tdir_count,
             (long unsigned)be32(entry->tdir_offset), (long unsigned)be32(entry->tdir_offset), (long unsigned)val);
#endif
...
int header_check_tiff_be(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if ((uint32_t)be32(header->tiff_diroff) < sizeof(TIFFHeader))
    return 0;
#if !defined(SINGLE_FORMAT) || defined(SINGLE_FORMAT_jpg)
  if (file_recovery->file_stat != NULL && file_recovery->file_stat->file_hint == &file_hint_jpg) {
