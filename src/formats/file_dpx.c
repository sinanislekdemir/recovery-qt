...
const file_hint_t file_hint_dpx = {.extension = "dpx",
...
static int header_check_dpx(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (memcmp(dpx->vers, ver10, sizeof(ver10)) == 0) {
    ...
    file_recovery_new->time = get_time_from_YYYY_MM_DD_HH_MM_SS((const unsigned char *)dpx->create_time);
    return 1;
  }
  return 0;
}
