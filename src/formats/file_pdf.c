...
const file_hint_t file_hint_pdf = {.extension = "pdf",
...
static void file_rename_pdf(file_recovery_t *file_recovery) {
  ...
  if (offset > (off_t)PHOTOREC_MAX_FILE_SIZE)
  ...
  if (buffer[i] == '<') {
    ...
    for (s = i + 1, j = i + 1; s + 1 < bsize && is_hexa(buffer[s]) && is_hexa(buffer[s + 1]); s += 2, j++)
  ...
  if (buffer[i] == '(') {
    ...
    if (i + 8 < bsize && memcmp(&buffer[i], "\\376\\377", 8) == 0) {
      ...
      while (i < bsize) {
        if (buffer[i] == ')')
          break;
        if (i + 4 < bsize && buffer[i] == '\\' && isdigit(buffer[i + 1]) && isdigit(buffer[i + 2]) &&
            isdigit(buffer[i + 3]))
          i += 4;
        else
          title[l++] = sbuffer[i++];
  ...
  if (l > 4 && (memcmp(&title[l - 4], ".doc", 4) == 0 || memcmp(&title[l - 4], ".xls", 4) == 0))
    l -= 4;
  else if (l > 5 && (memcmp(&title[l - 5], ".docx", 5) == 0 || memcmp(&title[l - 5], ".xlsx", 5) == 0))
    l -= 5;
...
static uint64_t read_pdf_file_aux(const unsigned char *buffer, unsigned int i) {
  ...
  while (i < 512 && (buffer[i] == ' ' || buffer[i] == '\t' || buffer[i] == '\n' || buffer[i] == '\r'))
    i++;
...
static int header_check_pdf(const unsigned char *buffer, const unsigned int buffer_size,
