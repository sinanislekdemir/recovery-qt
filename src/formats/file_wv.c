...
const file_hint_t file_hint_wv = {.extension = "wv",
...
static data_check_t data_check_wv(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
    ...
    if (memcmp(wv, wv_header, sizeof(wv_header)) == 0) {
      ...
      const uint64_t ape_tag_size =
          (buffer[i + 12] + (buffer[i + 13] << 8) + (buffer[i + 14] << 16) + ((uint64_t)buffer[i + 15] << 24)) + 32;
...
static int header_check_wv(const unsigned char *buffer, const unsigned int buffer_size,
