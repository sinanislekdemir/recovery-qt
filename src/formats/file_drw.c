...
const file_hint_t file_hint_drw = {.extension = "drw",
...
static void file_check_drw(file_recovery_t *file_recovery) {
  const unsigned char drw_footer[11] = {'#', 'E', 'N', 'D', '_', 'O', 'F', '_', 'U', 'G', 'C'};
  file_search_footer(file_recovery, drw_footer, sizeof(drw_footer), 1);
}

static int header_check_drw(const unsigned char *buffer, const unsigned int buffer_size,
...
static void register_header_check_drw(file_stat_t *file_stat) {
  static const unsigned char drw_header[14] = {'#', 'U', 'G', 'C', ':', '2', ' ', 'D', 'R', 'A', 'W', 'I', 'N', 'G'};
