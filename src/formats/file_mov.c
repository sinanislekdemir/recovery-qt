...
const file_hint_t file_hint_mov = {.extension = "mov",
...
const file_hint_t file_hint_mov_mdat = {.extension = "mov/mdat",
...
static data_check_t data_check_mov(const unsigned char *buffer, const unsigned int buffer_size,
	...
	return -((atom_size + buffer_size/2 - 1)/ (buffer_size/2));
      ...
      if (!(buffer[i + 4] == 0 && buffer[i + 5] == 0 && buffer[i + 6] == 0 && buffer[i + 7] == 0))
        log_warning("file_mov.c: unknown atom 0x%02x%02x%02x%02x at %llu\n", buffer[i + 4], buffer[i + 5],
                    buffer[i + 6], buffer[i + 7], (long long unsigned)file_recovery->calculated_file_size);
#endif
      return DC_STOP;
    }
  }
...
static int header_check_mov_aux(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (i <= buffer_size - 16) {
	...
	*/
    ...
    if (buffer[i + 4] == 'f' && buffer[i + 5] == 't' && buffer[i + 6] == 'y' && buffer[i + 7] == 'p') {
      ...
      if (memcmp(&buffer[i + 8], "isom", 4) == 0 || memcmp(&buffer[i + 8], "mp41", 4) == 0 ||
          memcmp(&buffer[i + 8], "mp42", 4) == 0 || memcmp(&buffer[i + 8], "mmp4", 4) == 0 ||
          memcmp(&buffer[i + 8], "M4B", 3) == 0 || memcmp(&buffer[i + 8], "M4P", 3) == 0 ||
    ...
    if (buffer[i + 4] == 'm' && buffer[i + 5] == 'd' && buffer[i + 6] == 'a' && buffer[i + 7] == 't') {
      if (memcmp(&buffer[i], "der.mdat\" anim=\"", 16) == 0)
        return 0;
      if (file_recovery->file_stat != NULL && file_recovery->file_check != NULL && buffer[8] == 'a' &&
          isprint(buffer[0]) && isprint(buffer[1]) && isprint(buffer[2]) && isprint(buffer[3])) {
...
static int header_check_mov(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  if (file_recovery->file_stat != NULL && file_recovery->file_check != NULL &&
      ...
      (file_recovery->calculated_file_size == file_recovery->file_size ||
       file_recovery->calculated_file_size == 0)) { /* PhotoRec is already trying to recover this mov file */
    header_ignored(file_recovery_new);
    return 0;
  }
