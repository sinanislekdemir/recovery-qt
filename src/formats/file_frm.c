...
const file_hint_t file_hint_frm = {.extension = "frm",
...
static void file_check_frm(file_recovery_t *file_recovery) {
  const unsigned char frm_footer[11] = {'#', 'E', 'N', 'D', '_', 'O', 'F', '_', 'U', 'G', 'C'};
  file_search_footer(file_recovery, frm_footer, sizeof(frm_footer), 1);
}

static int header_check_frm(const unsigned char *buffer, const unsigned int buffer_size,
...
static void register_header_check_frm(file_stat_t *file_stat) {
  static const unsigned char frm_header[14] = {'#', 'U', 'G', 'C', ':', '2', ' ', 'D', 'W', 'G', 'F', 'O', 'R', 'M'};
