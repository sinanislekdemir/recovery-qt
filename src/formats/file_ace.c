...
const file_hint_t file_hint_ace = {.extension = "ace",
...
static void file_check_ace(file_recovery_t *file_recovery) {
  ...
  while (!feof(file_recovery->handle)) {
    ...
    log_info("file_ace: Block header at 0x%08lx: CRC16=0x%04X size=%u type=%u"
             ...
             (long unsigned)file_recovery->file_size, le16(h->crc16), le16(h->size), h->type, le16(h->flags),
...
static int header_check_ace(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  file_recovery_new->min_filesize = 2 + /* CRC16 */
                                    2 + /* Head size */
                                    1 + /* Head type */
                                    2 + /* Flags */
                                    7 + /* Signature */
