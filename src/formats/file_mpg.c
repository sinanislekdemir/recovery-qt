...
const file_hint_t file_hint_mpg = {.extension = "mpg",
...
static unsigned int calculate_packet_size(const unsigned char *buffer) {
  ...
  /* Pack header: */
  case 0xBA:
    if ((buffer[4] & 0xc4) == 0x44 && (buffer[6] & 4) == 4 && (buffer[8] & 4) == 4 && (buffer[9] & 1) == 1 &&
        (buffer[12] & 3) == 3)
      return (buffer[13] & 0x7) + 14;
    if ((buffer[4] & 0xF1) == 0x21 && (buffer[6] & 1) == 1 && (buffer[8] & 1) == 1 && (buffer[9] & 0x80) == 0x80 &&
        (buffer[11] & 1) == 1)
      return 12;
    return 0;
    /* Sequence Header */
...
static data_check_t data_check_mpg(const unsigned char *buffer, const unsigned int buffer_size,
...
#ifdef DEBUG_MPG
    log_info("data_check_mpg %llu 0x%02x %u\n", (long long unsigned)file_recovery->calculated_file_size, buffer[i + 3],
             ret);
#endif
    if (ret == 0)
      return DC_STOP;
...
static int header_check_mpg_Pack(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if ((buffer[4] & 0xc4) == 0x44 && (buffer[6] & 4) == 4 && (buffer[8] & 4) == 4 && (buffer[9] & 1) == 1 &&
    ...
    if (buffer[4] == 0x44 && buffer[5] == 0 && buffer[6] == 4 && buffer[7] == 0 &&
        (buffer[8] & 0xfc) == 4) { /* SCR=0 */
      return header_mpg_found(file_recovery_new);
    }
    if (file_recovery->file_stat != NULL && file_recovery->file_check != NULL &&
        file_recovery->file_stat->file_hint == &file_hint_mpg) {
...
static int header_check_mpg_System(const unsigned char *buffer, const unsigned int buffer_size,
                                   const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                   file_recovery_t *file_recovery_new) {
  /* MPEG-1 http://andrewduncan.ws/MPEG/MPEG-1.ps */
...
static int header_check_mpg_Sequence(const unsigned char *buffer, const unsigned int buffer_size,
                                     const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                     file_recovery_t *file_recovery_new) {
...
static int header_check_mpg4_ElemVideo(const unsigned char *buffer, const unsigned int buffer_size,
                                       const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                                       file_recovery_t *file_recovery_new) {
