...
const file_hint_t file_hint_ari = {.extension = "ari",
...
static int header_check_ari(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  const struct arri_header *hdr = (const struct arri_header *)buffer;
  if (le32(hdr->version) == 0 || le32(hdr->width) == 0 || le32(hdr->height) == 0)
    return 0;
...
static void register_header_check_ari(file_stat_t *file_stat) {
  static const unsigned char ari_header[12] = {'A', 'R', 'R', 'I', 0x12, 0x34, 0x56, 0x78, 0x00, 0x10, 0x00, 0x00};
