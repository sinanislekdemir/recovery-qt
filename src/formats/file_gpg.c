...
const file_hint_t file_hint_gpg = {.extension = "gpg",
...
static unsigned int old_format_packet_length(const unsigned char *buf, unsigned int *length_type,
...
static unsigned int new_format_packet_length(const unsigned char *buf, unsigned int *length_type,
                                             int *partial_body_length) {
...
static unsigned int file_check_gpg_pubkey(FILE *handle, const uint64_t offset, const uint64_t tmp2) {
  ...
  if (my_fseek(handle, offset + tmp2, SEEK_SET) < 0 || fread(buffer, sizeof(buffer), 1, handle) != 1)
    return 0;
#ifdef __FRAMAC__
...
static void file_check_gpg(file_recovery_t *file_recovery) {
  ...
  while (stop == 0) {
	 ...
	 *          encrypted_session_key	*/
        if (buffer[i] == 3 && is_valid_pubkey_algo(pubkey_algo) && len > 0) {
          /* assert 0 < len <=2048; */
          ...
          log_info("GPG :pubkey enc packet: version %u, algo %u, keyid %02X%02X%02X%02X%02X%02X%02X%02X\n", buffer[i],
                   pubkey_algo, buffer[i + 1], buffer[i + 2], buffer[i + 3], buffer[i + 4], buffer[i + 5],
        ...
        if (buffer[i] != 1)
          return;
      } else if (tag == OPENPGP_TAG_PUB_KEY || tag == OPENPGP_TAG_PUB_SUBKEY || tag == OPENPGP_TAG_SEC_KEY ||
                 tag == OPENPGP_TAG_SEC_SUBKEY) {
        if ((buffer[i] == 2 || buffer[i] == 3) && is_valid_pubkey_algo(buffer[i + 1 + 4 + 2])) { /* version 2 or 3 */
        } else if (buffer[i] == 4 && is_valid_pubkey_algo(buffer[i + 1 + 4])) {                  /* version 4 */
        } else
          return;
      }
    }
...
static int header_check_gpg(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (nbr < 16 && i < buffer_size - 23 && stop == 0) {
	 ...
	 * uint8_t  pub_key_algo
	 *          encrypted_session_key	*/
        if (buffer[i] == 3 && is_valid_pubkey_algo(buffer[i + 1 + 8]) && len > 0) {
          const unsigned int offset_mpi = i + 1 + 8 + 1 + 2 + len;
#ifdef DEBUG_GPG
          log_info("GPG :pubkey enc packet: version %u, algo %u, keyid %02X%02X%02X%02X%02X%02X%02X%02X\n", buffer[i],
                   buffer[i + 1 + 8], buffer[i + 1], buffer[i + 2], buffer[i + 3], buffer[i + 4], buffer[i + 5],
                   buffer[i + 6], buffer[i + 7], buffer[i + 8]);
          log_info(" data: [ %u bits]\n", be16(*mpi_ptr));
#endif
          if (offset_mpi + 2 > length)
            return 0;
          if ((buffer[i + 1 + 8] == 16 || buffer[i + 1 + 8] == 20) && offset_mpi + 2 <= buffer_size) {
            int len2;
  ...
  /* encrypted_data.gpg */
  if (((packet_tag[0] == OPENPGP_TAG_PUBKEY_ENC_SESSION_KEY || packet_tag[0] == OPENPGP_TAG_SYMKEY_ENC_SESSION_KEY) &&
       (packet_tag[1] == OPENPGP_TAG_SYM_ENC_DATA || packet_tag[1] == OPENPGP_TAG_SYM_ENC_INTEGRITY)) ||
      /* pubring.gpg */
      (packet_tag[0] == OPENPGP_TAG_PUB_KEY && packet_tag[1] == OPENPGP_TAG_USER_ID &&
       packet_tag[2] == OPENPGP_TAG_SIGNATURE && packet_tag[3] == OPENPGP_TAG_TRUST) ||
      (packet_tag[0] == OPENPGP_TAG_PUB_KEY && packet_tag[1] == OPENPGP_TAG_USER_ID &&
       packet_tag[2] == OPENPGP_TAG_SIGNATURE && packet_tag[3] == OPENPGP_TAG_PUB_SUBKEY) ||
      /* secring.gpg */
      (packet_tag[0] == OPENPGP_TAG_SEC_KEY && packet_tag[1] == OPENPGP_TAG_USER_ID &&
       packet_tag[2] == OPENPGP_TAG_SIGNATURE && packet_tag[3] == OPENPGP_TAG_TRUST) ||
      (packet_tag[0] == OPENPGP_TAG_SEC_KEY && packet_tag[1] == 61 && packet_tag[2] == 61 && packet_tag[3] == 61)) {
    reset_file_recovery(file_recovery_new);
    file_recovery_new->file_check = &file_check_gpg;
