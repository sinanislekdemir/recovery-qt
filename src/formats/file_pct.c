...
const file_hint_t file_hint_pct = {.extension = "pct",
...
static void file_check_pct(file_recovery_t *file_recovery) {
  uint64_t diff;
  if (file_recovery->file_size < 0x210 || file_recovery->file_size < file_recovery->min_filesize) {
...
static int header_check_pct(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  const struct pct_file_entry *pct = (const struct pct_file_entry *)(&buffer[0x200]);
  if (be16(pct->XMin) <= be16(pct->XMax) && be16(pct->YMin) <= be16(pct->YMax) &&
      ((be16(pct->OXMin) <= be16(pct->OXMax) && be16(pct->OYMin) <= be16(pct->OYMax)) ||
      ...
      be16(pct->YMin) == 0 && be16(pct->OYMin) == 0 && be16(pct->VersionOperator) == 0x0011 &&
      be16(pct->VersionNumber) == 0x02ff) {
    const unsigned int min_filesize = (buffer[0x200] << 8) + buffer[0x201];
#if !defined(SINGLE_FORMAT)
    if (file_recovery->file_stat != NULL && file_recovery->file_stat->file_hint == &file_hint_indd) {
    ...
    file_recovery_new->min_filesize =
        (min_filesize > 0x200 + sizeof(struct pct_file_entry) ? min_filesize : 0x200 + sizeof(struct pct_file_entry));
    file_recovery_new->file_check = &file_check_pct;
#ifdef DEBUG_PCT
    log_info("X %u-%u, Y %u-%u\n", be16(pct->XMin), be16(pct->XMax), be16(pct->YMin), be16(pct->YMax));
    log_info("X %u-%u, Y %u-%u\n", be16(pct->OXMin), be16(pct->OXMax), be16(pct->OYMin), be16(pct->OYMax));
#endif
    return 1;
  }
