...
const file_hint_t file_hint_m2ts = {.extension = "m2ts",
...
const file_hint_t file_hint_ts = {.extension = "ts",
...
static data_check_t data_check_ts_192(const unsigned char *buffer, const unsigned int buffer_size,
...
static void file_rename_ts_188(file_recovery_t *file_recovery) {
  ...
  if ((file = fopen(file_recovery->filename, "rb")) == NULL)
    return;
  if (my_fseek(file, 0, SEEK_SET) < 0 || fread(&buffer, sizeof(buffer), 1, file) != 1) {
    fclose(file);
    return;
  }
  fclose(file);
...
static int header_check_m2ts(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (file_recovery->file_stat != NULL && file_recovery->file_check != NULL &&
      file_recovery->file_stat->file_hint == &file_hint_m2ts && file_recovery->data_check == &data_check_ts_192) {
...
static data_check_t data_check_ts_188(const unsigned char *buffer, const unsigned int buffer_size,
                                      file_recovery_t *file_recovery) {
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
...
static int header_check_m2t(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  unsigned int i;
  if (file_recovery->file_stat != NULL && file_recovery->data_check == &data_check_ts_188 &&
