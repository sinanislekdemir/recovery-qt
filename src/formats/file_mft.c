...
const file_hint_t file_hint_mft = {.extension = "mft",
...
static int header_check_mft(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (!(memcmp(buffer, "FILE", 4) == 0 && usa_ofs + usa_count <= attrs_offset && 42 <= attrs_offset &&
        attrs_offset % 8 == 0 && attrs_offset < bytes_in_use && bytes_in_use <= bytes_allocated))
    return 0;
