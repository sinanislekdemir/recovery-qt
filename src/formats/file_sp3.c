...
const file_hint_t file_hint_sp3 = {.extension = "sp3",
...
static time_t get_time_from_sp3(const struct SP3FileInfo *h) {
  ...
  if (DataExameAno > 1960 && DataExameAno < 2100 && h->DataExameMes >= 1 && h->DataExameMes <= 12 &&
...
static int header_check_sp3(const unsigned char *buffer, const unsigned int buffer_size,
