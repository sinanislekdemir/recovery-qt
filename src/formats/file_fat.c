...
const file_hint_t file_hint_fat = {.extension = "fat",
                                   .description = "FAT",
...
static int header_check_fat_aux(file_recovery_t *file_recovery_new, const unsigned int part_size,
...
static int header_check_fat(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  start_data =
      start_fat1 + fat_header->fats * fat_length + (get_dir_entries(fat_header) * 32 + sector_size - 1) / sector_size;
