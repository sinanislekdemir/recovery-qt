...
const file_hint_t file_hint_class = {.extension = "class",
...
static int header_check_class(const unsigned char *buffer, const unsigned int buffer_size,
                              const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                              file_recovery_t *file_recovery_new) {
  const struct class_header *cafe = (const struct class_header *)buffer;
  if (be32(cafe->magic_number) == 0xCafeBabe && be16(cafe->major_version) >= 45 && be16(cafe->major_version) <= 100 &&
