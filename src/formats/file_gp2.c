...
const file_hint_t file_hint_gp2 = {.extension = "gp2", /* .gp, .gobe */
...
static void file_check_gp2(file_recovery_t *file_recovery) {
  const unsigned char gp2_footer[8] = {0x00, 0x00, 0x00, 0x01, 'E', 'N', 'D', '!'};
  file_search_footer(file_recovery, gp2_footer, sizeof(gp2_footer), 0);
}

static int header_check_gp2(const unsigned char *buffer, const unsigned int buffer_size,
...
static void register_header_check_gp2(file_stat_t *file_stat) {
  static const unsigned char gp2_header[20] = {'G', 'O', 'B', 'E', ' ', 'S', 'C', 'O', 'T', 'T',
                                               '/', '0', '1', '/', '0', '2', 'D', 'O', 'C', '!'};
