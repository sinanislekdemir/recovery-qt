...
const file_hint_t file_hint_icns = {.extension = "icns",
...
static int check_icon_type(const char *type) {
  ...
  if (memcmp(type, "ICON", 4) == 0 || memcmp(type, "ICN#", 4) == 0 || memcmp(type, "icm#", 4) == 0 ||
      memcmp(type, "icm4", 4) == 0 || memcmp(type, "icm8", 4) == 0 || memcmp(type, "ics#", 4) == 0 ||
      memcmp(type, "ics4", 4) == 0 || memcmp(type, "ics8", 4) == 0 || memcmp(type, "is32", 4) == 0 ||
      memcmp(type, "s8mk", 4) == 0 || memcmp(type, "icl4", 4) == 0 || memcmp(type, "icl8", 4) == 0 ||
      memcmp(type, "il32", 4) == 0 || memcmp(type, "l8mk", 4) == 0 || memcmp(type, "ich#", 4) == 0 ||
      memcmp(type, "ich4", 4) == 0 || memcmp(type, "ich8", 4) == 0 || memcmp(type, "ih32", 4) == 0 ||
      memcmp(type, "h8mk", 4) == 0 || memcmp(type, "it32", 4) == 0 || memcmp(type, "t8mk", 4) == 0 ||
      memcmp(type, "icp4", 4) == 0 || memcmp(type, "icp5", 4) == 0 || memcmp(type, "icp6", 4) == 0 ||
      memcmp(type, "ic07", 4) == 0 || memcmp(type, "ic08", 4) == 0 || memcmp(type, "ic09", 4) == 0 ||
      memcmp(type, "ic10", 4) == 0 || memcmp(type, "ic11", 4) == 0 || memcmp(type, "ic12", 4) == 0 ||
      memcmp(type, "ic13", 4) == 0 || memcmp(type, "ic14", 4) == 0 || memcmp(type, "TOC ", 4) == 0 ||
      memcmp(type, "icnV", 4) == 0)
    return 1;
  return 0;
}

static int header_check_icns(const unsigned char *buffer, const unsigned int buffer_size,
