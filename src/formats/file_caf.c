...
const file_hint_t file_hint_caf = {.extension = "caf",
...
static data_check_t data_check_caf(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
    ...
    log_trace("file_caf.c: %s chunk %c%c%c%c (0x%02x%02x%02x%02x) size %llu, calculated_file_size %llu (0x%llx)\n",
              ...
              buffer[i + 2], buffer[i + 3], (long long unsigned)chunk_size,
...
static int header_check_caf(const unsigned char *buffer, const unsigned int buffer_size,
...
static void register_header_check_caf(file_stat_t *file_stat) {
  static const unsigned char caf_header[12] = {'c', 'a', 'f', 'f', 0x00, 0x01, 0x00, 0x00, 'd', 'e', 's', 'c'};
