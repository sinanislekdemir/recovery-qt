...
const file_hint_t file_hint_mp3 = {.extension = "mp3",
...
static unsigned int pos_in_mem(const unsigned char *haystack, const unsigned int haystack_size,
                               const unsigned char *needle, const unsigned int needle_size) {
  unsigned int i;
...
static unsigned int search_MMT(const unsigned char *buffer, const unsigned int i, const unsigned int buffer_size) {
  ...
  const unsigned char mm_pad_version_info[14] = {0x00, 0x00, 0x00, 0x00, '1', '8',  '2',
                                                 '7',  '3',  '6',  '4',  '5', 0x00, 0x00};
  ...
  if (memcmp(&buffer[i], mm_header, sizeof(mm_header)) == 0) // Optional Header
  {
    ...
    if (memcmp(&buffer[i], "    ", 4) != 0 && memcmp(&buffer[i], "bmp ", 4) != 0 && memcmp(&buffer[i], "jpg ", 4) != 0)
      return 0;
  ...
  }
  {
    ...
    if (tmp + sizeof(mm_pad_version_info) >
        buffer_size) { /* FIXME: Is it better to have a partial MusicMatch Tag or none ? */
...
static data_check_t data_check_mp3(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
    ...
    } else if (buffer[i] == 'L' && buffer[i + 1] == 'Y' && buffer[i + 2] == 'R' && buffer[i + 3] == 'I' &&
               buffer[i + 4] == 'C' && buffer[i + 5] == 'S' && buffer[i + 6] == 'B' && buffer[i + 7] == 'E' &&
               buffer[i + 8] == 'G' && buffer[i + 9] == 'I' && buffer[i + 10] == 'N') {
      /*
	 ...
	 */
        ...
        buffer[i] == 'A' && buffer[i + 1] == 'P' && buffer[i + 2] == 'E' && buffer[i + 3] == 'T' &&
        buffer[i + 4] == 'A' && buffer[i + 5] == 'G' && buffer[i + 6] == 'E' &&
        buffer[i + 7] ==
            'X') { /* APE Tagv2 (APE Tagv1 has no header) http://wiki.hydrogenaudio.org/index.php?title=APE_Tags_Header */
      const uint64_t ape_tag_size =
          (buffer[i + 12] | (buffer[i + 13] << 8) | (buffer[i + 14] << 16) | ((uint64_t)buffer[i + 15] << 24)) +
          (uint64_t)32;

      file_recovery->calculated_file_size += ape_tag_size;

    } else if (buffer[i] == 'T' && buffer[i + 1] == 'A' &&
               buffer[i + 2] == 'G') { /* http://www.id3.org/ID3v1 TAGv1 size = 128 bytes with header "TAG" */
      file_recovery->calculated_file_size += 128;

    } else if (buffer[i] == 'I' && buffer[i + 1] == 'D' && buffer[i + 2] == '3' &&
               (buffer[i + 3] == 2 || buffer[i + 3] == 3 || buffer[i + 3] == 4) && buffer[i + 4] == 0) {
...
static data_check_t data_check_id3(const unsigned char *buffer, const unsigned int buffer_size,
                                   file_recovery_t *file_recovery) {
  while (file_recovery->calculated_file_size + buffer_size / 2 >= file_recovery->file_size &&
         file_recovery->calculated_file_size + 1 < file_recovery->file_size + buffer_size / 2) {
    const unsigned int i = file_recovery->calculated_file_size + buffer_size / 2 - file_recovery->file_size;

    if (buffer[i] == 0) { /* Padding is present */
      file_recovery->calculated_file_size++;
    } else { /* no more padding or no padding */
...
static int header_check_id3(const unsigned char *buffer, const unsigned int buffer_size,
  ...
  if (buffer[0] == 'I' && buffer[1] == 'D' && buffer[2] == '3' &&
     ...
     */
    if (buffer[3] == 4 &&
        (buffer[5] & 0x10) == 0x10) /* a footer is present http://www.id3.org/id3v2.4.0-structure chap. 3.1 */
      potential_frame_offset = 10;

    potential_frame_offset +=
        ((buffer[6] & 0x7f) << 21) + ((buffer[7] & 0x7f) << 14) + ((buffer[8] & 0x7f) << 7) + (buffer[9] & 0x7f) + 10;
...
static int header_check_mp3(const unsigned char *buffer, const unsigned int buffer_size,
                            const unsigned int safe_header_only, const file_recovery_t *file_recovery,
                            file_recovery_t *file_recovery_new) {
  unsigned int potential_frame_offset = 0;
  unsigned int nbr = 0;
  ...
  */
  if (!(buffer[0] == 0xFF && ((buffer[1] & 0xFE) == 0xFA || (buffer[1] & 0xFE) == 0xF2 || (buffer[1] & 0xFE) == 0xE2)))
    return 0;

  while (potential_frame_offset + 1 < buffer_size && potential_frame_offset + 1 < 8192) {
  ...
  if (file_recovery->file_stat != NULL && file_recovery->file_check != NULL) {
    ...
    if (file_recovery->file_stat->file_hint == &file_hint_tiff && buffer[0] == buffer[3] && buffer[1] == buffer[4] &&
        buffer[2] == buffer[5]) {
