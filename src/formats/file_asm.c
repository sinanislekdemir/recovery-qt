...
const file_hint_t file_hint_asm = {.extension = "asm",
...
static void file_check_asm(file_recovery_t *file_recovery) {
  const unsigned char asm_footer[11] = {'#', 'E', 'N', 'D', '_', 'O', 'F', '_', 'U', 'G', 'C'};
  file_search_footer(file_recovery, asm_footer, sizeof(asm_footer), 1);
}

static int header_check_asm(const unsigned char *buffer, const unsigned int buffer_size,
...
static void register_header_check_asm(file_stat_t *file_stat) {
  static const unsigned char asm_header[16] = {'#', 'U', 'G', 'C', ':', '2', ' ', 'A',
                                               'S', 'S', 'E', 'M', 'B', 'L', 'Y', ' '};
