...
const file_hint_t file_hint_lnk = {.extension = "lnk",
...
static unsigned int lnk_get_size(const unsigned char *buffer, const unsigned int buffer_size) {
  ...
  if (i >= buffer_size - 4)
    return 0;
  if ((flags & SLDF_HAS_ID_LIST) != 0) { /* The Shell Item Id List */
  ...
  if (i >= buffer_size - 4)
    return 0;
  if ((flags & SLDF_HAS_LINK_INFO) != 0) { /* File location info */
  ...
  if (i >= buffer_size - 2)
    return 0;
  if ((flags & SLDF_HAS_NAME) != 0) { /* Description string */
    const uint16_t *ptr = (const uint16_t *)&buffer[i];
  ...
  if (i >= buffer_size - 2)
    return 0;
  if ((flags & SLDF_HAS_RELPATH) != 0) { /* Relative path */
    const uint16_t *ptr = (const uint16_t *)&buffer[i];
    unsigned int len = le16(*ptr);
    if ((flags & SLDF_UNICODE) != 0)
      len *= 2;
  ...
  if (i >= buffer_size - 2)
    return 0;
  if ((flags & SLDF_HAS_WORKINGDIR) != 0) { /* Working directory */
    const uint16_t *ptr = (const uint16_t *)&buffer[i];
    unsigned int len = le16(*ptr);
    if ((flags & SLDF_UNICODE) != 0)
      len *= 2;
  ...
  if (i >= buffer_size - 2)
    return 0;
  if ((flags & SLDF_HAS_ARGS) != 0) { /* Command line string */
    const uint16_t *ptr = (const uint16_t *)&buffer[i];
    unsigned int len = le16(*ptr);
    if ((flags & SLDF_UNICODE) != 0)
      len *= 2;
  ...
  if (i >= buffer_size - 2)
    return 0;
  if ((flags & SLDF_HAS_ICONLOCATION) != 0) { /* Icon filename string */
    const uint16_t *ptr = (const uint16_t *)&buffer[i];
    unsigned int len = le16(*ptr);
    if ((flags & SLDF_UNICODE) != 0)
      len *= 2;
...
static int header_check_lnk(const unsigned char *buffer, const unsigned int buffer_size,
...
static void register_header_check_lnk(file_stat_t *file_stat) {
  static const unsigned char lnk_header[20] = {
      ...
      0x01, 0x14, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46 /* GUID */
  };
