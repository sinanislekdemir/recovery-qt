...
.PHONY: help linux win linux-universal format lint clean

help:
	@echo "recovery-qt build targets:"
	@echo ""
	@echo "  make linux            - native Linux build into build/ (build/recovery-qt)"
	@echo "  make linux-universal  - portable Linux tarball via Docker into build/ (build/recovery-qt-*.tar.gz)"
